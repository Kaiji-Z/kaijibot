...
- Extension production code should import from `kaijibot/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `kaijibot.plugin.json` and the package
  `kaijibot` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `kaijibot/plugin-sdk/*`.
