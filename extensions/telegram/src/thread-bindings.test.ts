...
vi.mock("kaijibot/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("kaijibot/plugin-sdk/acp-runtime")>(
    "kaijibot/plugin-sdk/acp-runtime",
  );
...
vi.mock("kaijibot/plugin-sdk/json-store", async () => {
  const actual = await vi.importActual<typeof import("kaijibot/plugin-sdk/json-store")>(
    "kaijibot/plugin-sdk/json-store",
  );
...
describe("telegram thread bindings", () => {
  ...
  beforeEach(async () => {
    ...
    const acpRuntime = await vi.importActual<typeof import("kaijibot/plugin-sdk/acp-runtime")>(
      "kaijibot/plugin-sdk/acp-runtime",
    );
  ...
  it("does not persist lifecycle updates when manager persistence is disabled", async () => {
    stateDirOverride = fs.mkdtempSync(path.join(os.tmpdir(), "kaijibot-telegram-bindings-"));
  ...
  it("persists unbinds before restart so removed bindings do not come back", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:kaijibot-codex-app-server:abc123",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
  ...
  it("keeps plugin-owned bindings when ACP cleanup runs on startup", async () => {
    ...
    await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:kaijibot-codex-app-server:still-valid",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
    ...
    expect(reloaded.getByConversationId("plugin-binding-convo")?.targetSessionKey).toBe(
      "plugin-binding:kaijibot-codex-app-server:still-valid",
    );
