...
} from "kaijibot/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "kaijibot/plugin-sdk/setup-tools";
...
export function shouldShowTelegramDmAccessWarning(cfg: KaijiBotConfig, accountId: string): boolean {
...
export function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`kaijibot config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`kaijibot config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
