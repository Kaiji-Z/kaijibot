import { resolveMarkdownTableMode } from "kaijibot/plugin-sdk/config-runtime";
import {
...
const { loadConfig, resolveStorePath } = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn(
    (storePath?: string) => storePath ?? "/tmp/kaijibot-telegram-send-tests.json",
  ),
}));
...
vi.mock("kaijibot/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("kaijibot/plugin-sdk/config-runtime")>(
...
export function installTelegramSendTestHooks() {
  beforeEach(() => {
    ...
    resolveStorePath.mockReturnValue("/tmp/kaijibot-telegram-send-tests.json");
