...
import {
  ...
  type MsgContext,
} from "kaijibot/plugin-sdk/reply-runtime";
import type { MockFn } from "kaijibot/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("kaijibot/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("kaijibot/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("kaijibot/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("kaijibot/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/kaijibot-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("kaijibot/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: KaijiBotConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: KaijiBotConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: KaijiBotConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: KaijiBotConfig) => {
