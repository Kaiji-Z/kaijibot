...
} from "kaijibot/plugin-sdk/account-id";
...
function resolveDefaultAgentId(cfg: KaijiBotConfig): string {
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  const chosen = (agents.find((agent) => agent?.default) ?? agents[0])?.id;
  return normalizeAgentId(chosen);
}

function listConfiguredAccountIds(cfg: KaijiBotConfig): string[] {
...
function listBoundAccountIds(cfg: KaijiBotConfig, channelId: string): string[] {
  const ids = new Set<string>();
...
function resolveDefaultAgentBoundAccountId(cfg: KaijiBotConfig, channelId: string): string | null {
...
export function listTelegramAccountIds(cfg: KaijiBotConfig): string[] {
...
export function resolveDefaultTelegramAccountSelection(cfg: KaijiBotConfig): {
  accountId: string;
  accountIds: string[];
...
export function resolveDefaultTelegramAccountId(cfg: KaijiBotConfig): string {
