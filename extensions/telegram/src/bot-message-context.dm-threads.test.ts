...
const { recordInboundSessionMock, resolveStorePathMock } = vi.hoisted(() => ({
  ...
  resolveStorePathMock: vi.fn<ResolveStorePathFn>(() => "/tmp/kaijibot-session-store.json"),
}));
...
const { clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
  await import("kaijibot/plugin-sdk/config-runtime");
...
afterEach(() => {
  ...
  resolveStorePathMock.mockReturnValue("/tmp/kaijibot-session-store.json");
});
...
describe("buildTelegramMessageContext group sessions without forum", () => {
  ...
  it("reloads topic name from disk after cache reset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-telegram-topic-name-"));
...
describe("buildTelegramMessageContext direct peer routing", () => {
  it("isolates dm sessions by sender id when chat id differs", async () => {
    const runtimeCfg = {
      agents: { defaults: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/kaijibot" } },
      channels: { telegram: {} },
