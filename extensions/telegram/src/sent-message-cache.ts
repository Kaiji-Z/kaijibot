...
import { resolveStorePath, type KaijiBotConfig } from "kaijibot/plugin-sdk/config-runtime";
import { logVerbose } from "kaijibot/plugin-sdk/runtime-env";

const TTL_MS = 24 * 60 * 60 * 1000;
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("kaijibot.telegramSentMessagesState");
...
function resolveSentMessageStorePath(cfg?: Pick<KaijiBotConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<KaijiBotConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<KaijiBotConfig, "session">): SentMessageStore {
...
export function recordSentMessage(
  ...
  messageId: number,
  cfg?: Pick<KaijiBotConfig, "session">,
): void {
