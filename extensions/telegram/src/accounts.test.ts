...
import * as runtimeEnvModule from "kaijibot/plugin-sdk/runtime-env";
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): KaijiBotConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): KaijiBotConfig => ({
    channels: {
      telegram: {
