...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "kaijibot-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<KaijiBotConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<KaijiBotConfig["channels"]>["telegram"]>>,
): KaijiBotConfig {
  return {
    channels: {
      telegram: {
