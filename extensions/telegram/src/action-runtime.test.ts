...
import { captureEnv } from "kaijibot/plugin-sdk/testing";
...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): KaijiBotConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as KaijiBotConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): KaijiBotConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as KaijiBotConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: KaijiBotConfig, accountId = "media") {
    await handleTelegramAction(
