...
Use this skill when the knowledge-wiki vault render mode is `obsidian` or the user wants the wiki to play nicely with Obsidian.

- Start from `kaijibot wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `kaijibot wiki obsidian status` before shelling out, then prefer the dedicated helpers like `kaijibot wiki obsidian search`, `kaijibot wiki obsidian open`, `kaijibot wiki obsidian command`, and `kaijibot wiki obsidian daily`.
