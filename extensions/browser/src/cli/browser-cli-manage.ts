...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "kaijibot";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "kaijibot"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "kaijibot" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (kaijibot|existing-session). Default: kaijibot")
    .action(
      async (
        opts: {
          name: string;
