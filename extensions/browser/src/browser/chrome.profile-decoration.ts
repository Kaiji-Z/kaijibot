...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".kaijibot-profile-decorated");
}
 ...
 */
export function decorateKaijiBotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_KAIJIBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_KAIJIBOT_BROWSER_COLOR).toUpperCase();
