...
describe("browser remote profile fallback and attachOnly behavior", () => {
  it("uses profile-level attachOnly when global attachOnly is false", async () => {
    const state = deps.makeState("kaijibot");
    ...
    const launchMock = vi.mocked(deps.chromeModule.launchKaijiBotChrome);
    const ctx = deps.createBrowserRouteContext({ getState: () => state });

    await expect(ctx.forProfile("kaijibot").ensureBrowserAvailable()).rejects.toThrow(
  ...
  it("does not enforce managed tab cap for remote kaijibot profiles", async () => {
