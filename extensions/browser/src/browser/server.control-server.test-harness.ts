...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "kaijibot" | "existing-session";
      attachOnly?: boolean;
    }
  >;
...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.KAIJIBOT_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "kaijibot",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/kaijibot" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    kaijibot: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchKaijiBotChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "kaijibot";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.KAIJIBOT_GATEWAY_PORT;
  process.env.KAIJIBOT_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.KAIJIBOT_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.KAIJIBOT_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.KAIJIBOT_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.KAIJIBOT_GATEWAY_PASSWORD;
  } else {
    process.env.KAIJIBOT_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
