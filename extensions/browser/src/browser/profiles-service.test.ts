...
import { resolveKaijiBotUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveKaijiBotUserDataDir: vi.fn(() => "/tmp/kaijibot-test/kaijibot/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "kaijibot-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveKaijiBotUserDataDir).mockReturnValue(userDataDir);
