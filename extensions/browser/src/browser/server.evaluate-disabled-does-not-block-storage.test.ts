  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          kaijibot: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.KAIJIBOT_GATEWAY_PORT;
    process.env.KAIJIBOT_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.KAIJIBOT_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.KAIJIBOT_GATEWAY_PASSWORD;
