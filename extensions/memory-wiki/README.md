# @kaijibot/memory-wiki

Persistent wiki compiler and Obsidian-friendly knowledge vault for **KaijiBot**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.kaijibot-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
```bash
kaijibot wiki status
kaijibot wiki doctor
kaijibot wiki init
kaijibot wiki ingest ./notes/alpha.md
kaijibot wiki compile
kaijibot wiki lint
kaijibot wiki search "alpha"
kaijibot wiki get entity.alpha --from 1 --lines 80

kaijibot wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

kaijibot wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

kaijibot wiki bridge import
kaijibot wiki unsafe-local import

kaijibot wiki obsidian status
kaijibot wiki obsidian search "alpha"
kaijibot wiki obsidian open syntheses/alpha-summary.md
kaijibot wiki obsidian command workspace:quick-switcher
kaijibot wiki obsidian daily
```
...
- The compiled agent digests in `.kaijibot-wiki/cache/agent-digest.json` and `.kaijibot-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
