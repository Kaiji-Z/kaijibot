...
const AGENT_DIGEST_PATH = ".kaijibot-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".kaijibot-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- kaijibot:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- kaijibot:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- kaijibot:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- kaijibot:wiki:${group.dir}:index:end -->`,
