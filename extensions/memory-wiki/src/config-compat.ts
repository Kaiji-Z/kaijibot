...
export const legacyConfigRules: LegacyConfigRule[] = [
  {
    ...
    message:
      'plugins.entries.memory-wiki.config.bridge.readMemoryCore is legacy; use plugins.entries.memory-wiki.config.bridge.readMemoryArtifacts. Run "kaijibot doctor --fix".',
    match: hasLegacyBridgeArtifactToggle,
  },
];

export function migrateMemoryWikiLegacyConfig(config: KaijiBotConfig): {
...
export function normalizeCompatibilityConfig({ cfg }: { cfg: KaijiBotConfig }): {
  config: KaijiBotConfig;
  changes: string[];
} {
