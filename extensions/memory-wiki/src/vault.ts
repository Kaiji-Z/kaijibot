...
import {
  ...
  withTrailingNewline,
} from "kaijibot/plugin-sdk/memory-host-markdown";
...
function buildIndexMarkdown(): string {
  return withTrailingNewline(
    replaceManagedMarkdownBlock({
      original: "# Wiki Index\n",
      heading: "## Generated",
      startMarker: "<!-- kaijibot:wiki:index:start -->",
      endMarker: "<!-- kaijibot:wiki:index:end -->",
...
- Use \`.kaijibot-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the KaijiBot memory-wiki plugin.
...
- \`.kaijibot-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.

## Notes
<!-- kaijibot:human:start -->
<!-- kaijibot:human:end -->
`);
}
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(
    path.join(rootDir, ".kaijibot-wiki", "state.json"),
    withTrailingNewline(
      JSON.stringify(
        {
          version: 1,
  ...
  await writeFileIfMissing(path.join(rootDir, ".kaijibot-wiki", "log.jsonl"), "", createdFiles);
