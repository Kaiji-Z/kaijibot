...
import { resolveMemoryRemDreamingConfig } from "kaijibot/plugin-sdk/memory-core-host-status";
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-rem-harness-"));
...
function formatDreamingSummary(cfg: KaijiBotConfig): string {
...
export async function runMemoryRemBackfill(opts: MemoryRemBackfillOptions) {
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-rem-backfill-"));
      try {
