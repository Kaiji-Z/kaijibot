...
import {
  ...
  resolveShortTermPromotionConfig,
...
function resolveMemoryPluginConfig(cfg: KaijiBotConfig): Record<string, unknown> {
...
function resolveAgent(cfg: KaijiBotConfig, agent?: string) {
...
function resolveAgentIds(cfg: KaijiBotConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: kaijibot memory status --fix --agent ${agentId}`)}`);
      }
    }
...
export async function runMemorySearch(
  ...
  const shortTermConfig = resolveShortTermPromotionConfig({
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      void recordShortTermRecalls({
        ...
        query,
        results,
        timezone: shortTermConfig.timezone,
      }).catch(() => {
...
export async function runMemoryPromote(opts: MemoryPromoteCommandOptions) {
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const promotionConfig = resolveShortTermPromotionConfig({
        pluginConfig: resolveMemoryPluginConfig(cfg),
        cfg,
      });
      if (!workspaceDir) {
      ...
      try {
        candidates = await rankShortTermPromotionCandidates({
          workspaceDir,
          limit: opts.limit,
          minScore: opts.minScore ?? promotionConfig.minScore,
          minRecallCount: opts.minRecallCount ?? promotionConfig.minRecallCount,
          minUniqueQueries: opts.minUniqueQueries ?? promotionConfig.minUniqueQueries,
          recencyHalfLifeDays: promotionConfig.recencyHalfLifeDays,
          maxAgeDays: promotionConfig.maxAgeDays,
      ...
      if (opts.apply) {
        try {
          applyResult = await applyShortTermPromotions({
            ...
            minUniqueQueries: opts.minUniqueQueries ?? promotionConfig.minUniqueQueries,
            maxAgeDays: promotionConfig.maxAgeDays,
            timezone: promotionConfig.timezone,
          });
        } catch (err) {
...
export async function runMemoryPromoteExplain(
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const thresholds = {
        minScore: promotionConfig.minScore,
        minRecallCount: promotionConfig.minRecallCount,
        minUniqueQueries: promotionConfig.minUniqueQueries,
        maxAgeDays: promotionConfig.maxAgeDays ?? null,
      };
