...
function resolveStoredDreaming(config: KaijiBotConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: KaijiBotConfig = {}) {
  const registered: { command?: KaijiBotPluginCommandDefinition } = {};
  let runtimeConfig: KaijiBotConfig = initialConfig;

  const runtime = {
    config: {
      loadConfig: vi.fn(() => runtimeConfig),
      writeConfigFile: vi.fn(async (nextConfig: KaijiBotConfig) => {
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: KaijiBotPluginCommandDefinition) => {
      registered.command = definition;
    }),
  } as unknown as KaijiBotPluginApi;
