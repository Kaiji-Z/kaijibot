...
import { resolveSessionTranscriptsDirForAgent } from "kaijibot/plugin-sdk/memory-core";
import {
...
// Session file listing reads from the real KaijiBot state dir (~/.kaijibot/agents/).
// Mock it to return empty so tests don't ingest production session transcripts.
...
vi.mock("kaijibot/plugin-sdk/memory-core-host-engine-qmd", async (importOriginal) => {
  ...
  return {
    ...
    listSessionFilesForAgent: vi.fn(async (agentId: string) => {
      if (process.env.KAIJIBOT_STATE_DIR || process.env.OPENCLAW_STATE_DIR) {
        return original.listSessionFilesForAgent(agentId);
      }
      return [];
    }),
  };
...
const LIGHT_DREAMING_TEST_CONFIG: KaijiBotConfig = {
  plugins: {
    entries: {
      "memory-core": {
        config: {
...
async function createDreamingWorkspace(): Promise<string> {
  const workspaceDir = await createTempWorkspace("kaijibot-dreaming-phases-");
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__kaijibot_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
      ...
      await beforeAgentReply(
        {
          cleanedBody: [
            "System: rotate logs",
            "System: __kaijibot_memory_core_light_sleep__",
            "",
            "A scheduled reminder has been triggered. The reminder content is:",
            "",
            "rotate logs",
            "__kaijibot_memory_core_light_sleep__",
            "",
  ...
  it("checkpoints session transcript ingestion and skips unchanged transcripts", async () => {
    const workspaceDir = await createDreamingWorkspace();
    vi.stubEnv("KAIJIBOT_TEST_FAST", "1");
    vi.stubEnv("KAIJIBOT_STATE_DIR", path.join(workspaceDir, ".state"));
  ...
  it("records light/rem signals that reinforce deep promotion ranking", async () => {
    ...
    await withDreamingTestClock(async () => {
      ...
      await beforeAgentReply(
        { cleanedBody: "__kaijibot_memory_core_rem_sleep__" },
        { trigger: "heartbeat", workspaceDir },
      );
    });
