...
describe("TopicRegistry", () => {
  it("creates registry file on first upsert", async () => {
    ...
    expect(
      files.has(registryPath) || [...files.keys()].some((k) => k.includes("registry.json")),
    ).toBe(true);
  ...
  it("upsert creates new entry", async () => {
    ...
    await registry.upsertTopic(
      makeMeta({ name: "rust-lang", description: "Rust programming language topics" }),
    );
  ...
  it("refreshStats updates counts", async () => {
    const { registry } = createRegistry();
    await registry.upsertTopic(
      makeMeta({ name: "coding", entryCount: 3, lastUpdated: "2026-05-20" }),
    );
