...
import { resolveMemoryDreamingConfig } from "kaijibot/plugin-sdk/memory-core-host-status";
import { normalizeLowercaseStringOrEmpty } from "kaijibot/plugin-sdk/text-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: KaijiBotConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: KaijiBotConfig, enabled: boolean): KaijiBotConfig {
...
function formatStatus(cfg: KaijiBotConfig): string {
...
export function registerDreamingCommand(api: KaijiBotPluginApi): void {
  api.registerCommand({
    name: "dreaming",
