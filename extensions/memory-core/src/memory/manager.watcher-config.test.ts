...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): KaijiBotConfig {
    const defaults: NonNullable<NonNullable<KaijiBotConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: KaijiBotConfig) {
