...
function normalizeNotifyState(raw: unknown): NotifyStateFile {
  ...
  for (const item of subscribersRaw) {
    ...
    const channel = normalizeOptionalString(record.channel) ?? undefined;
...
export async function handleNotifyCommand(params: {
  ...
  if (params.ctx.channel !== "feishu") {
    return { text: "Pairing notifications are supported on Feishu." };
  }

  const target = resolveNotifyTarget(params.ctx);
  if (!target) {
    return { text: "Could not resolve notification target for this chat." };
  }
  ...
  if (params.action === "on" || params.action === "enable") {
    ...
    return {
      text:
        "✅ Pair request notifications enabled for this chat.\n" +
  ...
  if (params.action === "off" || params.action === "disable") {
    ...
    return { text: "✅ Pair request notifications disabled for this chat." };
  }
  ...
  if (params.action === "once" || params.action === "arm") {
    ...
    return {
      text:
        "✅ One-shot pairing notification armed for this chat.\n" +
...
export function registerPairingNotifierService(api: KaijiBotPluginApi): void {
