...
`kaijibot.invoke --each`) without adding new KaijiBot code per workflow.
...
This extension depends on KaijiBot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** KaijiBot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.kaijibot/extensions` as a standalone plugin directory.
