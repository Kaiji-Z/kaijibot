...
describe('CardKitApiError', () => {
  it('stores code and msg', () => {
    const err = new CardKitApiError({ api: 'create', code: 230099, msg: 'bad card', context: 'test' });
...
describe('extractSubCode', () => {
  it('extracts ErrCode from msg string', () => {
    const msg = 'Failed to create card content, ext=ErrCode: 11310; ErrMsg: element exceeds the limit; code:230099';
...
describe('isCardRateLimitError', () => {
  ...
  it('works with Axios shape', () => {
    expect(isCardRateLimitError({ response: { data: { code: 230020, msg: 'rate limited' } } })).toBe(true);
  });
});
