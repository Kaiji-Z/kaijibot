...
// ---------------------------------------------------------------------------
// extractLarkApiCode — extract Feishu API error code from thrown objects
// ---------------------------------------------------------------------------

/**
 * 从任意抛错对象中提取飞书 API 错误码。
 *
 * 支持三种形状：
 *   1. `{ code: number }` — 直接顶层
 *   2. `{ data: { code: number } }` — 嵌套 data
 ...
 */
export function parseCardApiError(err: unknown): {
  code: number;
  subCode: number | null;
  errMsg: string;
} | null {
