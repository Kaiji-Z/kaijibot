...
export async function handleWhatsAppStart(host: KaijiBotApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: KaijiBotApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: KaijiBotApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: KaijiBotApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: KaijiBotApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: KaijiBotApp): string {
...
function resolveGatewayHttpAuthHeader(host: KaijiBotApp): string | null {
...
function buildGatewayHttpHeaders(host: KaijiBotApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: KaijiBotApp) {
...
export function handleNostrProfileToggleAdvanced(host: KaijiBotApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: KaijiBotApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: KaijiBotApp) {
  const state = host.nostrProfileFormState;
