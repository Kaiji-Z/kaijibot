  ...
  return html`
    <button
      ...
      title=${state.canReveal
        ? state.isRevealed
          ? t("settings.hideValue")
          : t("settings.revealValue")
        : t("settings.disableStreamToReveal")}
...
export function renderNode(params: {
  ...
  if (unsupported.has(key)) {
    return html`<div class="cfg-field cfg-field--error">
      ...
      <div class="cfg-field__error">${t("settings.unsupportedNode")}</div>
    </div>`;
  }
  if (
    criteria &&
...
function renderTextInput(params: {
  ...
  const placeholder = effectiveRedacted
    ? isStructuredSecretRef
      ? rawAvailable
        ? t("settings.structuredSecretRefRaw")
...
function renderSelect(params: {
  ...
  return html`
    <div class="cfg-field">
      ...
      >
        <option value=${unset}>${t("settings.selectDots")}</option>
    ...
    <div class="cfg-field">
      ...
      <div class="cfg-input-wrap">
        <textarea
          ...
          placeholder=${sensitiveState.isRedacted ? REDACTED_PLACEHOLDER : t("settings.jsonValue")}
          rows="3"
          .value=${displayValue}
          ?disabled=${disabled}
...
function renderArray(params: {
  ...
  if (!itemsSchema) {
    return html`
      <div class="cfg-field cfg-field--error">
        <div class="cfg-field__label">${label}</div>
        <div class="cfg-field__error">${t("settings.unsupportedArray")}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="cfg-array">
      ...
      ${arr.length === 0
        ? html` <div class="cfg-array__empty">${t("settings.noItemsYet")}</div> `
        : html`
  ...
  return html`
    <div class="cfg-map">
      <div class="cfg-map__header">
        <span class="cfg-map__label">${t("settings.customEntries")}</span>
        <button
          type="button"
          class="cfg-map__add"
          ?disabled=${disabled}
      ...
      ${visibleEntries.length === 0
        ? html` <div class="cfg-map__empty">${t("settings.noCustomEntries")}</div> `
        : html`
