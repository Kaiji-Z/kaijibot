...
export async function loadUsageCost(state: UsageDashboardState) {
  ...
  try {
    const [cost, sessions, status] = await Promise.all([
      state.client
        .request("usage.cost", { days: 30 })
        .catch((e) => (console.warn("usage.cost", e), null)),
      state.client
        .request("sessions.usage", { limit: 50 })
        .catch((e) => (console.warn("sessions.usage", e), null)),
      state.client
        .request("usage.status", {})
        .catch((e) => (console.warn("usage.status", e), null)),
    ]);
    ...
    state.usageDashboardError =
      failures > 0 ? `${failures} of 3 usage RPCs failed — data may be incomplete` : null;
  } catch (err) {
