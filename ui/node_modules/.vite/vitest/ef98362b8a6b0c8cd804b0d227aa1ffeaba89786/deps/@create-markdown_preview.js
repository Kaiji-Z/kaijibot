...
//#region ../node_modules/@create-markdown/preview/dist/index.js
...
async function lazyParse(markdown) {
	try {
		return (await import("./preview-MsM0ZJB7.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1))).parse(markdown);
	} catch {
		throw new Error("@create-markdown/core is required for markdownToHTML(). Install it as a dependency, or use applyPreviewTheme() with your own parser instead.");
	}
}
var DEFAULT_OPTIONS = {
	classPrefix: "cm-",
...
function blocksToHTML(blocks, options) {
	...
	for (const plugin of opts.plugins) if (plugin.transformBlock) transformedBlocks = transformedBlocks.map(plugin.transformBlock);
	let html = `<div class="${prefix}preview">${transformedBlocks.map((block) => {
		for (const plugin of opts.plugins) if (plugin.renderBlock) {
			const result = plugin.renderBlock(block, () => renderBlock(block, opts));
			if (result !== null) return result;
		}
		const customRenderer = opts.customRenderers[block.type];
		if (customRenderer) return customRenderer(block);
...
async function markdownToHTML(markdown, options) {
	return blocksToHTML(await lazyParse(markdown), options);
}
async function renderAsync(blocks, options) {
	const opts = resolveOptions(options);
	for (const plugin of opts.plugins) if (plugin.init) await plugin.init();
	let html = blocksToHTML(blocks, options);
	for (const plugin of opts.plugins) if (plugin.postProcess) html = await plugin.postProcess(html);
	return html;
}
function renderBlock(block, opts) {
	const prefix = opts.classPrefix;
	switch (block.type) {
		case "paragraph": return `<p class="${prefix}paragraph">${renderInlineContent(block.content, opts)}</p>`;
		case "heading": return renderHeading(block, opts);
		case "bulletList": return renderBulletList(block, opts);
		case "numberedList": return renderNumberedList(block, opts);
		case "checkList": return renderCheckList(block, opts);
		case "codeBlock": return renderCodeBlock(block, opts);
		case "blockquote": return `<blockquote class="${prefix}blockquote">${renderInlineContent(block.content, opts)}</blockquote>`;
		case "table": return renderTable(block, opts);
		case "image": return renderImage(block, opts);
		case "divider": return `<hr class="${prefix}divider" />`;
		case "callout": return renderCallout(block, opts);
		default: return `<div class="${prefix}unknown">${renderInlineContent(block.content, opts)}</div>`;
	}
}
function renderHeading(block, opts) {
	...
	const tag = `h${level}`;
	return `<${tag} class="${prefix}heading ${prefix}h${level}">${renderInlineContent(block.content, opts)}</${tag}>`;
}
function renderBulletList(block, opts) {
	return `<ul class="${opts.classPrefix}bullet-list">
${block.children.map((child) => `<li>${renderInlineContent(child.content, opts)}</li>`).join("\n")}
</ul>`;
}
function renderNumberedList(block, opts) {
	return `<ol class="${opts.classPrefix}numbered-list">
${block.children.map((child) => `<li>${renderInlineContent(child.content, opts)}</li>`).join("\n")}
</ol>`;
}
...
<div class="${prefix}checklist-item">
  ...
  <span class="${checked ? `${prefix}checked` : ""}">${renderInlineContent(block.content, opts)}</span>
</div>`.trim();
}
function renderCodeBlock(block, opts) {
	...
	return `<pre class="${prefix}code-block"${language ? ` data-language="${language}"` : ""}><code class="${prefix}code${langClass}">${escapedCode}</code></pre>`;
}
...
${headers.length > 0 ? `<thead><tr>${headers.map((h, i) => `<th${getStyle(i)}>${escapeHtml(h)}</th>`).join("")}</tr></thead>` : ""}
<tbody>
${rows.map((row) => `<tr>${row.map((cell, i) => `<td${getStyle(i)}>${escapeHtml(cell)}</td>`).join("")}</tr>`).join("\n")}
</tbody>
</table>`;
}
function renderImage(block, opts) {
	...
	return `<figure class="${prefix}image">${`<img src="${escapeHtml(url)}"${altAttr}${titleAttr}${widthAttr}${heightAttr} />`}${alt ? `<figcaption>${escapeHtml(alt)}</figcaption>` : ""}</figure>`;
}
...
<div class="${prefix}callout ${prefix}callout-${type}" role="alert">
  ...
  <div class="${prefix}callout-content">${renderInlineContent(block.content, opts)}</div>
</div>`.trim();
}
...
function renderSpan(span, opts) {
	...
	if (styles.highlight) html = `<mark>${html}</mark>`;
	if (styles.strikethrough) html = `<del>${html}</del>`;
...
function buildTagMappings(prefix) {
	return [
		...
		{
			tag: "ul",
			classes: [`${prefix}bullet-list`]
		},
		{
			tag: "ol",
			classes: [`${prefix}numbered-list`]
		},
		{
			tag: "pre",
			classes: [`${prefix}code-block`]
		},
		{
			tag: "blockquote",
			classes: [`${prefix}blockquote`]
		},
		{
			tag: "hr",
			classes: [`${prefix}divider`]
		},
		{
			tag: "table",
			classes: [`${prefix}table`]
		},
		{
			tag: "figure",
			classes: [`${prefix}image`]
		}
	];
}
function injectClasses(openTag, classes) {
	...
	if (match) return openTag.replace(classAttrRe, `class="${classStr} ${match[1]}"`);
	if (openTag.endsWith("/>")) return openTag.slice(0, -2) + ` class="${classStr}" />`;
	return openTag.slice(0, -1) + ` class="${classStr}">`;
}
function wrapStandaloneImages(html, prefix) {
	return html.replace(/(?<!<figure[^>]*>\s*)(<img\s[^>]*\/?>)(?!\s*<\/figure>)/gi, `<figure class="${prefix}image">$1</figure>`);
}
function applyPreviewTheme(html, options) {
	...
	if (typeof options?.sanitize === "function") result = options.sanitize(result);
	return result;
}
var themeCSS = {
	githubDark: `/**
...
function shikiPlugin(options) {
	...
	return {
		...
		async init() {
			...
			try {
				const createHighlighter = (await import("./dist-BTU4WxfM.js")).createHighlighter;
...
function mermaidPlugin(options) {
	...
	return {
		...
		async init() {
			...
			try {
				const mermaid = await import("./preview-bY-Tcmoc.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
		...
		renderBlock(block, _defaultRender) {
			if (block.type !== "codeBlock") return null;
			const codeBlock = block;
			if (codeBlock.props.language?.toLowerCase() !== "mermaid") return null;
			const code = codeBlock.content.map((span) => span.text).join("");
			const prefix = opts.classPrefix;
			return `
<div class="${prefix}mermaid-container">
  <pre class="${prefix}mermaid" id="${opts.useUniqueIds ? `mermaid-${Date.now()}-${++diagramCounter}` : `mermaid-${++diagramCounter}`}">${escapeHtml2(code)}</pre>
</div>`.trim();
		},
		async postProcess(html) {
			if (!mermaidModule) return html;
			try {
				const mermaid = mermaidModule;
				const prefix = opts.classPrefix;
				const mermaidBlockRegex = new RegExp(`<pre class="${prefix}mermaid" id="([^"]+)">([\\s\\S]*?)</pre>`, "g");
				...
				for (const match of matches) {
					...
					try {
						...
						html = html.replace(fullMatch, `<div class="${prefix}mermaid-diagram">${svg}</div>`);
					} catch (renderError) {
...
async function lazyParse2(markdown) {
	try {
		...
		throw new Error("@create-markdown/core is required to parse markdown in <markdown-preview>. Install it, or provide pre-parsed blocks via the blocks attribute / setBlocks().");
	}
}
var MarkdownPreviewElement = class extends HTMLElement {
	constructor() {
		...
		if (mode !== "none") this._shadow = this.attachShadow({ mode });
	...
	getMarkdown() {
		...
		if (blocksAttr) try {
			return JSON.parse(blocksAttr).map((b) => b.content.map((s) => s.text).join("")).join("\n\n");
		} catch {
			return "";
		}
	...
	async getBlocks() {
		...
		return lazyParse2(this.textContent || "");
	}
	async render() {
		...
		try {
			let html;
			if (isAsync) html = await renderAsync(blocks, options);
			else html = blocksToHTML(blocks, options);
...
function autoRegister() {
	if (typeof window !== "undefined" && typeof customElements !== "undefined") registerPreviewElement();
}
var VERSION = "2.0.0";
//#endregion
export { MarkdownPreviewElement, VERSION, applyPreviewTheme, autoRegister, blocksToHTML, createMermaidPlugin, createShikiPlugin, getThemePath, markdownToHTML, mermaidPlugin, registerPreviewElement, renderAsync, shikiPlugin, themeNames, themeCSS as themes };

//# sourceMappingURL=@create-markdown_preview.js.map