...
- Known gaps: `src/process/**` (12 test files) is excluded from all vitest projects; some paths under `src/infra/` remain outside the configured projects — use `pnpm tsgo` for type verification, or create a temporary vitest config to run tests locally. `src/gateway/**` tests run via `corepack pnpm vitest run --config vitest/vitest.gateway.config.ts <path>`. The former `src/commands/**` gap is closed: `vitest/vitest.commands.config.ts` exists (isolation forced — these tests mutate registries/env and false-fail non-isolated), is wired into `rootVitestProjects` and `fullSuiteVitestShards`, and `pnpm test src/commands/<path>` routes to it.
