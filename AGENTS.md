...
This is a simplified fork of [KaijiBot](https://github.com/kaijibot/kaijibot), targeting Chinese Windows users with Feishu + Z.AI (智谱 GLM). The upstream AGENTS.md has been adapted; only guidance relevant to this fork is kept.

- Repo: fork of `https://github.com/kaijibot/kaijibot`, upstream synced from `https://gitee.com/kaiji1126/kaijibot`
- In chat replies, file references must be repo-root relative only (e.g. `src/cli/index.ts:80`); never absolute paths or `~/...`.

## Project Structure

- **`src/`** — core engine: CLI (`src/cli`), commands (`src/commands`), gateway (`src/gateway`), agents (`src/agents`), config (`src/config`), plugin system (`src/plugins`, `src/plugin-sdk`), channels (`src/channels`), media pipeline (`src/media`)
- **`extensions/`** — 16 bundled plugins (workspace packages). The only messaging channel is **feishu**; the primary LLM provider is **zai**. Also includes: openai, browser, memory-core, memory-lancedb, memory-wiki, speech-core, talk-voice, media-understanding-core, image-generation-core, diffs, llm-task, device-pair, webhooks, shared
...
- Scoped test: `pnpm test <path-or-filter>` (e.g. `pnpm test src/commands/onboard-search.test.ts`)
...
- Default model: `zai/glm-5.1`. Set via `kaijibot config set agent.model "zai/glm-5.1"`.
...
Core code (`src/`) is fully compatible; merge conflicts should be rare.
