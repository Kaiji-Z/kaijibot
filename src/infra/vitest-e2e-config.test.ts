...
import { BUNDLED_PLUGIN_E2E_TEST_GLOB } from "../../vitest/vitest.bundled-plugin-paths.ts";
import e2eConfig from "../../vitest/vitest.e2e.config.ts";
...
describe("e2e vitest config", () => {
  ...
  it("includes e2e test globs and runtime setup", () => {
    ...
    expect(e2eConfig.test?.setupFiles).toContain("test/setup-kaijibot-runtime.ts");
  });
});
