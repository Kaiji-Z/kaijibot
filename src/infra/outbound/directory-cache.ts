...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: KaijiBotConfig | null = null;
  ...
  get(key: string, cfg: KaijiBotConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: KaijiBotConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: KaijiBotConfig): void {
