...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): KaijiBotConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): KaijiBotConfig {
  return {
