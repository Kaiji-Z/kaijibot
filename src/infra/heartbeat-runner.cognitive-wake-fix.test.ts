...
describe.skipIf(process.env.CI)("HeartbeatRunner cognitive wake bypass for non-default agents", () => {
  ...
  it("routes cognitive-insight wake to non-default agent (state.agents bypass)", async () => {
    ...
    const runSpy = vi.fn().mockResolvedValue({ status: "ran", durationMs: 1 }) as unknown as RunOnce;
