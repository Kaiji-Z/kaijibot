...
import { kaijiBotRootFs, kaijiBotRootFsSync } from "./kaijibot-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["kaijibot"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  try {
    return parsePackageName(await kaijiBotRootFs.readFile(path.join(dir, "package.json"), "utf-8"));
  } catch {
    return null;
  }
}
...
function readPackageNameSync(dir: string): string | null {
  try {
    return parsePackageName(
      kaijiBotRootFsSync.readFileSync(path.join(dir, "package.json"), "utf-8"),
    );
  } catch {
    return null;
  }
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = kaijiBotRootFsSync.realpathSync(normalized);
...
export async function resolveKaijiBotPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveKaijiBotPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
