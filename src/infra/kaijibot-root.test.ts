...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__kaijibot_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "kaijibot-root");
...
function setPackageRoot(root: string, name = "kaijibot") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./kaijibot-root.js").resolveKaijiBotPackageRootSync,
  asyncResolver: typeof import("./kaijibot-root.js").resolveKaijiBotPackageRoot,
  opts: Parameters<typeof import("./kaijibot-root.js").resolveKaijiBotPackageRootSync>[0],
...
vi.mock("./kaijibot-root.fs.runtime.js", () => ({
  kaijiBotRootFsSync: mockFsModule(),
  kaijiBotRootFs: mockFsPromisesModule(),
}));

describe("resolveKaijiBotPackageRoot", () => {
  let resolveKaijiBotPackageRoot: typeof import("./kaijibot-root.js").resolveKaijiBotPackageRoot;
  let resolveKaijiBotPackageRootSync: typeof import("./kaijibot-root.js").resolveKaijiBotPackageRootSync;
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ resolveKaijiBotPackageRoot, resolveKaijiBotPackageRootSync } =
      await import("./kaijibot-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "kaijibot");
        const pkgRoot = path.join(project, "node_modules", "kaijibot");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "kaijibot");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "kaijibot.mjs")));
    ...
    {
      name: "falls through from a non-kaijibot moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-kaijibot");
    ...
    {
      name: "returns null for non-kaijibot package roots",
      setup: () => {
        const pkgRoot = fx("not-kaijibot");
        setPackageRoot(pkgRoot, "not-kaijibot");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "kaijibot.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "kaijibot");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveKaijiBotPackageRootSync,
      resolveKaijiBotPackageRoot,
      opts,
      expected,
    );
  });
