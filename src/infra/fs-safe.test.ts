...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("kaijibot-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("kaijibot-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("kaijibot-fs-safe-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("kaijibot-fs-safe-root-");
    const sourceDir = await tempDirs.make("kaijibot-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("kaijibot-fs-safe-root-");
    const outside = await tempDirs.make("kaijibot-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    const originalKaijiBotHome = process.env.KAIJIBOT_HOME;
    const fakeHome = path.resolve(path.sep, "tmp", "fake-home-test");
    process.env.HOME = fakeHome;
    process.env.KAIJIBOT_HOME = fakeHome;
    try {
      const result = expandHomePrefix("~/file.txt");
      expect(path.normalize(result)).toBe(path.join(fakeHome, "file.txt"));
    } finally {
      process.env.HOME = originalHome;
      process.env.KAIJIBOT_HOME = originalKaijiBotHome;
    }

    const root = await tempDirs.make("kaijibot-tilde-test-");
    process.env.HOME = root;
    process.env.KAIJIBOT_HOME = root;
    try {
    ...
    const outsideRoot = await tempDirs.make("kaijibot-tilde-outside-");
    await expect(
      openFileWithinRoot({
        rootDir: outsideRoot,
        relativePath: "~/escape.txt",
