...
export const POSIX_KAIJIBOT_TMP_DIR = "/tmp/kaijibot";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredKaijiBotTmpDirOptions = {
...
export function resolvePreferredKaijiBotTmpDir(
  options: ResolvePreferredKaijiBotTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "kaijibot" : `kaijibot-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[kaijibot] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback KaijiBot temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback KaijiBot temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_KAIJIBOT_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_KAIJIBOT_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_KAIJIBOT_TMP_DIR)) {
      return POSIX_KAIJIBOT_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_KAIJIBOT_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_KAIJIBOT_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_KAIJIBOT_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_KAIJIBOT_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_KAIJIBOT_TMP_DIR;
