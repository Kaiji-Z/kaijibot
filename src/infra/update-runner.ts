...
const DEFAULT_PACKAGE_NAME = "kaijibot";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const PREFLIGHT_TEMP_PREFIX =
  process.platform === "win32" ? "ocu-pf-" : "kaijibot-update-preflight-";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["kaijibot", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    try {
      ...
      const doctorEntry = path.join(gitRoot, "kaijibot.mjs");
      ...
      const doctorStep = await runStep(
        step("kaijibot doctor", doctorArgv, gitRoot, { KAIJIBOT_UPDATE_IN_PROGRESS: "1" }),
      );
