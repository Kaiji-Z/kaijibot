...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.KAIJIBOT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*kaijibot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "kaijibot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "kaijibot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "kaijibot-gw");
  ...
  records.push(`_kaijibot-gw._tcp IN PTR ${instanceLabel}._kaijibot-gw._tcp`);
  records.push(`${instanceLabel}._kaijibot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._kaijibot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; kaijibot-content-hash: ${contentHash}\n${contentBody}`;
}
