...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.KAIJIBOT_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for KaijiBot. You can keep running the web gateway without it: \`${formatCliCommand("kaijibot gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
