...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.kaijibotCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.kaijibotCanvasA2UIAction &&
        typeof window.kaijibotCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.kaijibotSendUserAction === "function";
  ...
  window.addEventListener("kaijibot:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android KaijiBot node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.kaijibotSendUserAction === "function"
        ? window.kaijibotSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.KAIJIBOT_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.KAIJIBOT_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>KaijiBot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
