...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("kaijibot-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("kaijibot-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("kaijibot-marketplace-test-", async (homeDir) => {
      const kaijiBotHome = path.join(homeDir, "kaijibot-home");
      await fs.mkdir(path.join(homeDir, ".claude", "plugins"), { recursive: true });
      await fs.mkdir(kaijiBotHome, { recursive: true });
      await fs.writeFile(
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, KAIJIBOT_HOME: kaijiBotHome },
