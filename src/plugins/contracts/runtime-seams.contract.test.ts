...
function createInstalledRuntimePluginDir(
  ...
  const bundledDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `kaijibot-runtime-contract-bundled-${pluginId}-`),
  );
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `kaijibot-runtime-contract-state-${pluginId}-`),
  );
  ...
  fs.writeFileSync(
    path.join(pluginRoot, "kaijibot.plugin.json"),
    JSON.stringify({
      id: pluginId,
      channels: [pluginId],
