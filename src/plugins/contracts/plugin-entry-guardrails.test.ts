...
function analyzeSourceModule(params: { filePath: string; source: string }): {
  ...
  for (const statement of sourceFile.statements) {
    if (ts.isImportDeclaration(statement)) {
      ...
      if (
        specifier === "kaijibot/plugin-sdk/core" &&
    ...
    for (const plugin of listBundledPluginRoots()) {
      ...
      try {
        const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
          kaijibot?: { extensions?: unknown };
        };
        const extensions = Array.isArray(pkg.kaijibot?.extensions) ? pkg.kaijibot.extensions : [];
        if (
          extensions.some(
  ...
  it("follows relative import edges while scanning guarded contract graphs", () => {
    expect(
      analyzeSourceModule({
        ...
        import { y } from "kaijibot/plugin-sdk/testing";
      `,
  ...
  it("detects aliased definePluginEntry imports from core", () => {
    expect(
      analyzeSourceModule({
        ...
        source: `
          import { definePluginEntry as dpe } from "kaijibot/plugin-sdk/core";
          import { somethingElse } from "kaijibot/plugin-sdk/core";
        `,
      }).importsDefinePluginEntryFromCore,
    ).toBe(true);
  });
