...
const PLUGIN_SDK_SUBPATH_PATTERN = /kaijibot\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
function packKaijiBotToTempDir(packDir: string): string {
...
async function readPackedRootPackageJson(archivePath: string): Promise<{
  ...
  const extractDir = makeTrackedTempDir("kaijibot-packed-root-package-json", tempDirs);
  await tar.x({
    file: archivePath,
    cwd: extractDir,
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references kaijibot/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
