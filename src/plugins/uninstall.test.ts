...
type PluginConfig = NonNullable<KaijiBotConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: KaijiBotConfig["channels"];
}): KaijiBotConfig {
...
function createSinglePluginWithEmptySlotsConfig(): KaijiBotConfig {
...
function createSingleNpmInstallConfig(installPath: string): KaijiBotConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as KaijiBotConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "kaijibot-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-kaijibot-plugin-install/my-plugin",
      },
      extensionsDir,
    });
