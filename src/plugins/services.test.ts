...
import type { KaijiBotPluginService, KaijiBotPluginServiceContext } from "./types.js";
...
function createRegistry(services: KaijiBotPluginService[]) {
...
function expectServiceContext(
  ctx: KaijiBotPluginServiceContext,
...
function expectServiceLogger(ctx: KaijiBotPluginServiceContext) {
...
function expectServiceContexts(
  contexts: KaijiBotPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
async function startTrackingServices(params: {
  services: KaijiBotPluginService[];
  config?: Parameters<typeof startPluginServices>[0]["config"];
  workspaceDir?: string;
}) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: KaijiBotPluginServiceContext[] = [];
