...
describe("provider attribution", () => {
  it("resolves the canonical KaijiBot product and runtime version", () => {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { KAIJIBOT_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "kaijibot",
        version: "2026.3.22",
        "User-Agent": "kaijibot/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { KAIJIBOT_VERSION: "2026.3.22" })).toEqual({
      originator: "kaijibot",
      version: "2026.3.22",
      "User-Agent": "kaijibot/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { KAIJIBOT_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ KAIJIBOT_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
