...
import "./test-helpers/fast-kaijibot-tools-sessions.js";
...
function createKaijiBotTools(options?: {
...
describe("sessions tools", () => {
  ...
  it("uses number (not integer) in tool schemas for Gemini compatibility", () => {
    const tools = createKaijiBotTools();
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createKaijiBotTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createKaijiBotTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
