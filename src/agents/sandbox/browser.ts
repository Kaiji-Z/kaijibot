...
export async function ensureSandboxBrowser(params: {
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `kaijibot sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `kaijibot sandbox recreate --browser --agent ${agentId}`;
          }
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_KAIJIBOT_BROWSER_PROFILE_NAME)
    : null;
