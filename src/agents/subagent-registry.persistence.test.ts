...
describe("subagent registry persistence", () => {
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-subagent-"));
  ...
  it("uses isolated temp state when KAIJIBOT_STATE_DIR is unset in tests", async () => {
    delete process.env.KAIJIBOT_STATE_DIR;
    const registryPath = resolveSubagentRegistryPath();
    expect(registryPath).toContain(path.join(os.tmpdir(), "kaijibot-test-state"));
  });
});
