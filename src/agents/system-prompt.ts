...
const CONTEXT_FILE_ORDER = new Map<string, number>([
  ...
  ["bootstrap.md", 60],
  ["memory.md", 70],
  ["cognitive-persona.md", 80],
]);

const DYNAMIC_CONTEXT_FILE_BASENAMES = new Set(["heartbeat.md", "cognitive-persona.md"]);
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; KaijiBot handles all routing internally.",
...
function buildCapabilitiesSection(params: { isMinimal: boolean }): string[] {
  ...
  return [
    ...
    "- **PRISM Gate** — Cost-sensitive gating (signal detection theory): only reaches out when expected value (pNeed × pAccept) exceeds打扰 cost, with time-of-day and trust-phase hard rules",
    "- **Feedback Learning** — You learn from response length, topic continuity, and engagement depth to improve future interactions",
    "- **Trust Evolution** — Relationship lifecycle evolves through stages: new → active → dormant → lapsed (based on interaction frequency and recency)",
    "",
...
export function buildAgentSystemPrompt(params: {
  ...
  if (promptMode === "none") {
    return "You are a personal assistant operating inside KaijiBot.";
  }
  ...
  const lines = [
    ...
    ...safetySection,
    "## KaijiBot CLI Quick Reference",
    "KaijiBot is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- kaijibot gateway status",
    "- kaijibot gateway start",
    "- kaijibot gateway stop",
    "- kaijibot gateway restart",
    "If unsure, ask the user to run `kaijibot help` (or `kaijibot gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## KaijiBot Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, KaijiBot pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by KaijiBot and included below in Project Context.",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'KaijiBot treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
