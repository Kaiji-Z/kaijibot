...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("kaijibot-bundled-");
  const workspaceDir = await createTempDir("kaijibot-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: KaijiBotConfig) {
