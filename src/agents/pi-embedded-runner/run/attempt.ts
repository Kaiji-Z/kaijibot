...
import { resolveKaijiBotDocsPath } from "../../docs-path.js";
...
import { createKaijiBotCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
// [fork-removed] ollama-runtime.js re-exports removed
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveKaijiBotAgentDir();
    ...
    const toolsRaw = params.disableTools
      ? []
      : (() => {
          const allTools = createKaijiBotCodingTools({
            agentId: sessionAgentId,
    ...
    const docsPath = await resolveKaijiBotDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/kaijibot/kaijibot/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("kaijibot:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/kaijibot/kaijibot/issues/8643
