...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses KAIJIBOT_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      KAIJIBOT_HOME: "/srv/kaijibot-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/kaijibot-home"), ".kaijibot", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".kaijibot", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("kaijibot-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("kaijibot-workspace-");
    await fs.mkdir(path.join(tempDir, ".kaijibot"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-workspace-hardlink-"));
    try {
