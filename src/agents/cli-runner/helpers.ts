...
import { KeyedAsyncQueue } from "kaijibot/plugin-sdk/keyed-async-queue";
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredKaijiBotTmpDir(), "kaijibot-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".kaijibot-cli-images");
  }
  return path.join(resolvePreferredKaijiBotTmpDir(), "kaijibot-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredKaijiBotTmpDir(), "kaijibot-cli-system-prompt-"),
  );
