...
function resolveAttachmentLimits(config: KaijiBotConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".kaijibot", "attachments");
  const relDir = path.posix.join(".kaijibot", "attachments", attachmentId);
