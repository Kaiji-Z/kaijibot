...
function isTranscriptOnlyKaijiBotAssistantMessage(message: AgentMessage | undefined): boolean {
  if (!message || message.role !== "assistant") {
    return false;
  }
  const provider = normalizeOptionalString(message.provider) ?? "";
  const model = normalizeOptionalString(message.model) ?? "";
  return provider === "kaijibot" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
export function handleMessageStart(
  ...
  if (msg?.role !== "assistant" || isTranscriptOnlyKaijiBotAssistantMessage(msg)) {
    return;
  }
