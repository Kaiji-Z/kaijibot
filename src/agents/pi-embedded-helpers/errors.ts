...
function formatDiskSpaceErrorCopy(raw: string): string | undefined {
  ...
  if (
    ...
    return (
      "KaijiBot could not write local session data because the disk is full. " +
...
export function formatAssistantErrorText(
  ...
  opts?: { cfg?: KaijiBotConfig; sessionKey?: string; provider?: string; model?: string },
): string | undefined {
...
export function isModelNotFoundErrorMessage(raw: string): boolean {
  ...
  // Direct pattern matches from KaijiBot internals and common providers.
  if (
