...
function readCompatThinkingLevelMap(compat: unknown): Record<string, string> {
  if (!compat || typeof compat !== "object") {
    return {};
  }
  const rawMap = (compat as { thinkingLevelMap?: unknown }).thinkingLevelMap;
...
export function resolveOpenAIReasoningEffortMap(
  ...
  return {
    ...fallbackMap,
    ...builtinMap,
    ...readCompatThinkingLevelMap(model.compat),
  };
}
