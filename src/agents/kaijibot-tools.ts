...
import { resolveKaijiBotPluginToolsForOptions } from "./kaijibot-plugin-tools.js";
import {
  collectPresentKaijiBotTools,
  isUpdatePlanToolEnabledForKaijiBotTools,
} from "./kaijibot-tools.registration.js";
...
import { createMcpConfigTool } from "./tools/mcp-config-tool.js";
...
type KaijiBotToolsDeps = {
  callGateway: typeof callGateway;
  config?: KaijiBotConfig;
};

const defaultKaijiBotToolsDeps: KaijiBotToolsDeps = {
  callGateway,
};

let kaijiBotToolsDeps: KaijiBotToolsDeps = defaultKaijiBotToolsDeps;

export function createKaijiBotTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? kaijiBotToolsDeps.config;
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentKaijiBotTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    createGatewayTool({
      agentSessionKey: options?.agentSessionKey,
      config: options?.config,
    }),
    ...
    ...(isUpdatePlanToolEnabledForKaijiBotTools(resolvedConfig, options?.modelProvider)
    ...
    createSessionsListTool({
      ...
      sandboxed: options?.sandboxed,
      config: resolvedConfig,
      callGateway: kaijiBotToolsDeps.callGateway,
    }),
  ...
  const wrappedPluginTools = resolveKaijiBotPluginToolsForOptions({
    options,
    resolvedConfig,
...
export const __testing = {
  setDepsForTest(overrides?: Partial<KaijiBotToolsDeps>) {
    kaijiBotToolsDeps = overrides
      ? {
          ...defaultKaijiBotToolsDeps,
          ...overrides,
        }
      : defaultKaijiBotToolsDeps;
  },
