...
import { createKaijiBotReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createKaijiBotCodingTools read behavior", () => {
  it("applies sandbox path guards to canonical path", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "kaijibot-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createKaijiBotReadTool(
      baseRead as unknown as Parameters<typeof createKaijiBotReadTool>[0],
    );
