...
} from "./kaijibot-tools.subagents.sessions-spawn.test-harness.js";
import {
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.KAIJIBOT_TEST_FAST;
...
describe("kaijibot-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnAnnounceFlowOverride();
    resetSessionsSpawnHookRunnerOverride();
    resetSessionsSpawnConfigOverride();
  ...
  afterAll(() => {
    ...
    process.env.KAIJIBOT_TEST_FAST = fastModeEnv.previous;
  });
