...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "kaijibot-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.KAIJIBOT_BUNDLED_SKILLS_DIR = bundledDir;
