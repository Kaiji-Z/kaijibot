...
const DIRECT_ENABLED = Boolean(process.env.KAIJIBOT_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const LIVE_HEARTBEAT_MS = Math.max(1_000, toInt(process.env.KAIJIBOT_LIVE_HEARTBEAT_MS, 30_000));
const LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.KAIJIBOT_LIVE_SETUP_TIMEOUT_MS, 45_000),
);
  ...
  it(
    ...
    async () => {
      ...
      await withLiveStageTimeout(
        ensureKaijiBotModelsJson(cfg),
        "[live-models] prepare models.json",
      );
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set KAIJIBOT_LIVE_MODELS=modern|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const rawModels = process.env.KAIJIBOT_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.KAIJIBOT_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.KAIJIBOT_LIVE_MODEL_TIMEOUT_MS, 30_000);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via KAIJIBOT_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
