  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (includeArchived) {
        ...
        // Resolve transcript directories — one per unique agentId in live sessions
        ...
        if (storePath) {
          ...
          if (isUsable && trimmedStore.endsWith("sessions.json")) {
            dirs.add(path.dirname(path.resolve(trimmedStore)));
          }
        }
        ...
        for (const transcriptsDir of dirs) {
          try {
