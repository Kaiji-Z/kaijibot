...
describe("CronToolSchema", () => {
  ...
  it("job exposes the expected top-level fields", () => {
    expect(keysAt(CronToolSchema as unknown as Record<string, unknown>, "job")).toEqual(
      [
        "agentId",
        "deleteAfterRun",
        "delivery",
  ...
  it("patch exposes the expected top-level fields", () => {
    expect(keysAt(CronToolSchema as unknown as Record<string, unknown>, "patch")).toEqual(
      [
        "agentId",
        "deleteAfterRun",
        "delivery",
  ...
  it("job.schedule exposes kind, at, everyMs, anchorMs, expr, tz, staggerMs", () => {
    expect(keysAt(CronToolSchema as unknown as Record<string, unknown>, "job.schedule")).toEqual(
  ...
  it("marks staggerMs as cron-only in both job and patch schedule schemas", () => {
    const jobStagger = propertyAt(
      CronToolSchema as unknown as Record<string, unknown>,
  ...
  it("job.delivery exposes mode, channel, to, bestEffort, accountId, failureDestination", () => {
    expect(keysAt(CronToolSchema as unknown as Record<string, unknown>, "job.delivery")).toEqual(
  ...
  it("job.payload exposes kind, text, message, model, thinking and extras", () => {
    expect(keysAt(CronToolSchema as unknown as Record<string, unknown>, "job.payload")).toEqual(
      [
  ...
  it("patch.payload exposes agentTurn fallback overrides", () => {
    expect(keysAt(CronToolSchema as unknown as Record<string, unknown>, "patch.payload")).toEqual(
      [
        "allowUnsafeExternalContent",
        "fallbacks",
        "kind",
