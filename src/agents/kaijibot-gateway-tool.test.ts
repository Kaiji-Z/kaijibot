...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-test-"));

    try {
      await withEnvAsync(
        { KAIJIBOT_STATE_DIR: stateDir, KAIJIBOT_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: kaijibot --profile isolated doctor --non-interactive",
          );
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/kaijibot-bin"],
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/kaijibot" } } }',
      }),
    ).rejects.toThrow(
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/kaijibot-bin"] } } }',
      }),
    ).rejects.toThrow(
