...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousKaijiBotAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousKaijiBotAgentDir = process.env.KAIJIBOT_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousKaijiBotAgentDir === undefined) {
      delete process.env.KAIJIBOT_AGENT_DIR;
    } else {
      process.env.KAIJIBOT_AGENT_DIR = previousKaijiBotAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "KAIJIBOT_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.KAIJIBOT_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.KAIJIBOT_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.KAIJIBOT_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: KaijiBotConfig = {
  models: {
    providers: {
      "custom-proxy": {
