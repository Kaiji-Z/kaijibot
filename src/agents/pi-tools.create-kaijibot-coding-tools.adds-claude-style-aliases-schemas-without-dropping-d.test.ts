...
const defaultTools = createKaijiBotCodingTools();
...
describe("createKaijiBotCodingTools", () => {
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-read-"));
    try {
      ...
      const contents = "Hello from kaijibot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "kaijibot-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "kaijibot-workspace"),
    ...
    const tools = createKaijiBotCodingTools({ sandbox });
