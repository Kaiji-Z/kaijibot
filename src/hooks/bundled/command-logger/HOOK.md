...
homepage: https://gitee.com/kaiji1126/kaijibot/blob/main/docs/automation/hooks.md#command-logger
metadata:
  {
    "kaijibot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.kaijibot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.kaijibot/logs/commands.log ~/.kaijibot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/kaijibot`:
   ```
   /home/username/.kaijibot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
