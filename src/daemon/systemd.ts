...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  const override = env.KAIJIBOT_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.KAIJIBOT_PROFILE);
}
...
async function activateSystemdService(params: { env: GatewayServiceEnv }) {
  const serviceName = resolveGatewaySystemdServiceName(params.env.KAIJIBOT_PROFILE);
...
export async function uninstallSystemdService({
  ...
  await assertSystemdAvailable(env);
  const serviceName = resolveGatewaySystemdServiceName(env.KAIJIBOT_PROFILE);
  const unitName = `${serviceName}.service`;
