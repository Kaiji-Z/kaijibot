...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.KAIJIBOT_LIVE_ACP_BIND);
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.KAIJIBOT_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.KAIJIBOT_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.KAIJIBOT_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "kaijibot.json");
      ...
      process.env.KAIJIBOT_SKIP_CRON = "0";
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.KAIJIBOT_GATEWAY_PORT = previous.port;
        }
