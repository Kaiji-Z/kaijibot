...
describe("gateway pre-auth hardening", () => {
  ...
  it("closes idle unauthenticated sockets after the handshake timeout", async () => {
    const previous = process.env.KAIJIBOT_TEST_HANDSHAKE_TIMEOUT_MS;
    process.env.KAIJIBOT_TEST_HANDSHAKE_TIMEOUT_MS = "200";
    cleanupEnv.push(() => {
      if (previous === undefined) {
        delete process.env.KAIJIBOT_TEST_HANDSHAKE_TIMEOUT_MS;
      } else {
        process.env.KAIJIBOT_TEST_HANDSHAKE_TIMEOUT_MS = previous;
      }
    });
  ...
  it("rejects excess simultaneous unauthenticated sockets from the same client ip", async () => {
    const previous = process.env.KAIJIBOT_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP;
    process.env.KAIJIBOT_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP = "1";
    cleanupEnv.push(() => {
      if (previous === undefined) {
        delete process.env.KAIJIBOT_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP;
      } else {
        process.env.KAIJIBOT_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP = previous;
      }
    });
