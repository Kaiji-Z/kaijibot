...
type GatewaySecretDefaults = NonNullable<KaijiBotConfig["secrets"]>["defaults"];

export const trimToUndefined = normalizeOptionalString;

/**
 * Like trimToUndefined but also rejects unresolved env var placeholders (e.g. `${VAR}`).
 * This prevents literal placeholder strings like `${KAIJIBOT_GATEWAY_TOKEN}` from being
...
export function hasGatewayTokenEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.KAIJIBOT_GATEWAY_TOKEN));
}

export function hasGatewayPasswordEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.KAIJIBOT_GATEWAY_PASSWORD));
}
...
export function createGatewayCredentialPlan(params: {
  ...
  const envToken = trimToUndefined(env.KAIJIBOT_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.KAIJIBOT_GATEWAY_PASSWORD);
