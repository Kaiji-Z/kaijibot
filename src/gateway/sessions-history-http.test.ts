...
const READ_SCOPE_HEADER = { "x-kaijibot-scopes": "operator.read" };
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-session-history-"));
...
describe("session history HTTP endpoints", () => {
  test("returns session history over direct REST", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(
        (
          body.messages?.[0] as {
            __kaijibot?: { id?: string; seq?: number };
          }
        )?.__kaijibot,
      ).toMatchObject({
        seq: 1,
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: Array<{ content?: Array<{ text?: string }>; __kaijibot?: { seq?: number } }>;
        messages?: Array<{ content?: Array<{ text?: string }>; __kaijibot?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(firstBody.messages?.map((message) => message.__kaijibot?.seq)).toEqual([2, 3]);
      ...
      const secondBody = (await secondPage.json()) as {
        items?: Array<{ content?: Array<{ text?: string }>; __kaijibot?: { seq?: number } }>;
        messages?: Array<{ __kaijibot?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(secondBody.messages?.map((message) => message.__kaijibot?.seq)).toEqual([1]);
  ...
  test("streams bounded history windows over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(nextData.messages?.[0]?.__kaijibot).toMatchObject({
        id: thirdMessageId,
        seq: 3,
      });
  ...
  test("seeds bounded SSE windows from visible history when transcript refreshes are silent", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(refreshData.messages?.[0]?.__kaijibot?.seq).toBe(2);
  ...
  test("sanitizes phased assistant history entries before returning them", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(body.messages?.[0]?.__kaijibot).toMatchObject({
        id: visibleMessageId,
        seq: 2,
      });
    });
  ...
  test("streams session history updates over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(
        (
          messageEvent.data as {
            message?: { __kaijibot?: { id?: string; seq?: number } };
          }
        ).message?.__kaijibot,
      ).toMatchObject({
