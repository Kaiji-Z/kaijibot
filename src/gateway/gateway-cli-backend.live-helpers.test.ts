...
describe("gateway cli backend live helpers", () => {
  afterEach(() => {
    ...
    delete process.env.KAIJIBOT_SKIP_CHANNELS;
    delete process.env.KAIJIBOT_SKIP_PROVIDERS;
    delete process.env.KAIJIBOT_SKIP_GMAIL_WATCHER;
    delete process.env.KAIJIBOT_SKIP_CRON;
    delete process.env.KAIJIBOT_SKIP_CANVAS_HOST;
    delete process.env.KAIJIBOT_SKIP_BROWSER_CONTROL_SERVER;
    delete process.env.KAIJIBOT_BUNDLED_PLUGINS_DIR;
    delete process.env.KAIJIBOT_TEST_MINIMAL_GATEWAY;
  ...
  it("applies and restores live env including minimal gateway mode", async () => {
    ...
    process.env.KAIJIBOT_SKIP_CHANNELS = "old-channels";
    process.env.KAIJIBOT_SKIP_PROVIDERS = "old-providers";
    process.env.KAIJIBOT_SKIP_GMAIL_WATCHER = "old-gmail";
    process.env.KAIJIBOT_SKIP_CRON = "old-cron";
    process.env.KAIJIBOT_SKIP_CANVAS_HOST = "old-canvas";
    process.env.KAIJIBOT_SKIP_BROWSER_CONTROL_SERVER = "old-browser";
    process.env.KAIJIBOT_BUNDLED_PLUGINS_DIR = "old-bundled";
    process.env.KAIJIBOT_TEST_MINIMAL_GATEWAY = "old-minimal";
    ...
    expect(process.env.KAIJIBOT_SKIP_CHANNELS).toBe("1");
    expect(process.env.KAIJIBOT_SKIP_PROVIDERS).toBe("1");
    expect(process.env.KAIJIBOT_SKIP_GMAIL_WATCHER).toBe("1");
    expect(process.env.KAIJIBOT_SKIP_CRON).toBe("1");
    expect(process.env.KAIJIBOT_SKIP_CANVAS_HOST).toBe("1");
    expect(process.env.KAIJIBOT_SKIP_BROWSER_CONTROL_SERVER).toBe("1");
    expect(process.env.KAIJIBOT_BUNDLED_PLUGINS_DIR).toBe("old-bundled");
    expect(process.env.KAIJIBOT_TEST_MINIMAL_GATEWAY).toBe("1");
    expect(process.env.ANTHROPIC_API_KEY).toBeUndefined();
    expect(process.env.ANTHROPIC_API_KEY_OLD).toBeUndefined();

    restoreCliBackendLiveEnv(snapshot);

    expect(process.env.KAIJIBOT_SKIP_CHANNELS).toBe("old-channels");
    expect(process.env.KAIJIBOT_SKIP_PROVIDERS).toBe("old-providers");
    expect(process.env.KAIJIBOT_SKIP_GMAIL_WATCHER).toBe("old-gmail");
    expect(process.env.KAIJIBOT_SKIP_CRON).toBe("old-cron");
    expect(process.env.KAIJIBOT_SKIP_CANVAS_HOST).toBe("old-canvas");
    expect(process.env.KAIJIBOT_SKIP_BROWSER_CONTROL_SERVER).toBe("old-browser");
    expect(process.env.KAIJIBOT_BUNDLED_PLUGINS_DIR).toBe("old-bundled");
    expect(process.env.KAIJIBOT_TEST_MINIMAL_GATEWAY).toBe("old-minimal");
  ...
  it("defaults the model switch probe to Claude Sonnet -> Opus", async () => {
    ...
    delete process.env.KAIJIBOT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE;
  ...
  it("lets env disable the model switch probe", async () => {
    ...
    process.env.KAIJIBOT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE = "0";
