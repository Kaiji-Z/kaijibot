...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.KAIJIBOT_SKIP_CHANNELS;
    prevSkipGmail = process.env.KAIJIBOT_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.KAIJIBOT_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    prevGeminiApiKey = process.env.GEMINI_API_KEY;
    process.env.KAIJIBOT_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.KAIJIBOT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.KAIJIBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.KAIJIBOT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.KAIJIBOT_SKIP_PROVIDERS;
    } else {
      process.env.KAIJIBOT_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("KAIJIBOT_CONFIG_PATH is not set");
    }
  ...
  async function writeAuthProfileEnvRefStore() {
    ...
    if (!stateDir) {
      throw new Error("KAIJIBOT_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_KAIJIBOT_AUTH_REF" },
            },
          },
  ...
  it("fails startup when an active exec ref id contains traversal segments", async () => {
    ...
    const previousGatewayTokenEnv = process.env.KAIJIBOT_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.KAIJIBOT_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
    }
  });
  ...
  it("fails startup when auth-profile secret refs are unresolved", async () => {
    ...
    delete process.env.MISSING_KAIJIBOT_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_KAIJIBOT_AUTH_REF" is missing or empty.',
      );
    } finally {
  ...
  it("does not emit secrets reloader events for web search secret reload transitions", async () => {
    ...
    await withGatewayServer(async () => {
              ...
              config: {
                webSearch: {
                  apiKey: {
                    source: "env",
                    provider: "default",
                    id: "KAIJIBOT_TEST_MISSING_GEMINI_API_KEY",
                  },
                },
              },
            },
      ...
      delete process.env.GEMINI_API_KEY;
      delete process.env.KAIJIBOT_TEST_MISSING_GEMINI_API_KEY;
