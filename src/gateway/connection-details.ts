...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: KaijiBotConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : normalizeOptionalString(process.env.KAIJIBOT_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.KAIJIBOT_ALLOW_INSECURE_PRIVATE_WS === "1";
