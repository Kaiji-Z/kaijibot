...
describe("doctor.memory.status", () => {
  ...
  it("includes dreaming counts and managed cron status when workspace data is available", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((cfg: KaijiBotConfig, agentId: string) => {
      if (agentId === "alpha") {
    ...
    const cronList = vi.fn(async () => [
      {
        ...
        payload: {
          kind: "systemEvent",
          text: "__kaijibot_memory_core_short_term_promotion_dream__",
        },
  ...
  it("reads dreaming config from the selected memory slot plugin", async () => {
    loadConfig.mockReturnValue({
      plugins: {
        slots: {
          memory: "memos-local-kaijibot-plugin",
        },
        entries: {
          "memos-local-kaijibot-plugin": {
            config: {
              dreaming: {
                enabled: true,
                frequency: "0 */4 * * *",
  ...
  it("merges workspace store errors when multiple workspace stores are unreadable", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg: KaijiBotConfig, agentId: string) =>
