...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("KAIJIBOT_CONFIG_PATH is required for gateway config tests");
  }
...
async function postResponses(port: number, body: unknown, headers?: Record<string, string>) {
  const res = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
    ...
    headers: {
      ...
      "x-kaijibot-scopes": "operator.write",
      ...headers,
    },
...
describe("OpenResponses HTTP API (e2e)", () => {
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "kaijibot", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `kaijibot` or `kaijibot/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "kaijibot", input: "hi" },
        { "x-kaijibot-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "kaijibot/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "kaijibot/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "kaijibot", input: "hi" },
        { "x-kaijibot-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-kaijibot-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("treats write-scoped HTTP callers as non-owner and admin-scoped callers as owner", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "kaijibot", input: "hi" },
      { "x-kaijibot-scopes": "operator.admin, operator.write" },
    );
