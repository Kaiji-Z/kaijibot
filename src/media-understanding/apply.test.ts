...
const TEMP_MEDIA_PREFIX = "kaijibot-media-";
...
function createGroqAudioConfig(): KaijiBotConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): KaijiBotConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): KaijiBotConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: KaijiBotConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredKaijiBotTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        KAIJIBOT_AGENT_DIR: isolatedAgentDir,
