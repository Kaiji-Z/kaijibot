...
describe("image-generation runtime", () => {
  ...
  it("lists runtime image-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeImageGenerationProviders({ config: {} as KaijiBotConfig })).toEqual(
      providers,
    );
    expect(mocks.listImageGenerationProviders).toHaveBeenCalledWith({} as KaijiBotConfig);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    const promise = generateImage({ cfg: {} as KaijiBotConfig, prompt: "draw a cat" });
