...
function createApiKeyProvider(params: {
  ...
  noteMessage?: string;
  noteTitle?: string;
  applyConfig?: Partial<KaijiBotConfig>;
}): ProviderPlugin {
  return {
    ...
    auth: [
      createProviderApiKeyAuthMethod({
        ...
        ...(params.applyConfig ? { applyConfig: () => params.applyConfig as KaijiBotConfig } : {}),
        wizard: {
  ...
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as KaijiBotConfig,
...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    const env = await setupAuthTestEnv("kaijibot-auth-");
      ...
      config: {
        secrets: {
          providers: {
            filemain: {
              source: "file",
              path: "/tmp/kaijibot-missing-secrets.json",
              mode: "json",
            },
          },
        },
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireKaijiBotAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
