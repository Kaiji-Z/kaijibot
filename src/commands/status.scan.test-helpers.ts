...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/kaijibot-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = KaijiBotConfig>(
  overrides: T = {} as T,
): KaijiBotConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): KaijiBotConfig {
