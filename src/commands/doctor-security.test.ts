...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.KAIJIBOT_GATEWAY_TOKEN;
    prevPassword = process.env.KAIJIBOT_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    if (prevToken === undefined) {
      ...
      process.env.KAIJIBOT_GATEWAY_TOKEN = prevToken;
    }
    if (prevPassword === undefined) {
      delete process.env.KAIJIBOT_GATEWAY_PASSWORD;
    } else {
      process.env.KAIJIBOT_GATEWAY_PASSWORD = prevPassword;
    }
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".kaijibot"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".kaijibot", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as KaijiBotConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.KAIJIBOT_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as KaijiBotConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats SecretRef token config as authenticated for exposure warning level", async () => {
    const cfg = {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: { source: "env", provider: "default", id: "KAIJIBOT_GATEWAY_TOKEN" },
        },
      },
    } as KaijiBotConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as KaijiBotConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats unset bind as loopback for host-side doctor checks", async () => {
    const cfg = { gateway: {} } as KaijiBotConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("No channel security warnings detected");
    expect(message).not.toContain("Gateway bound");
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as KaijiBotConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("kaijibot approvals get --gateway");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as KaijiBotConfig);
    const message = lastMessage();
    expect(message).toContain("[secrets]");
    expect(message).toContain("failed to resolve account");
    expect(message).toContain("Run: kaijibot security audit --deep");
  });
