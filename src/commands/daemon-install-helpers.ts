...
const MANAGED_SERVICE_ENV_KEYS_VAR = "KAIJIBOT_SERVICE_MANAGED_ENV_KEYS";
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      process.platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(params.env.KAIJIBOT_PROFILE)
        : undefined,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("kaijibot gateway install")}\` after fixing the error.`;
}
