...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveKaijiBotAgentDir: vi.fn().mockReturnValue("/tmp/kaijibot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/kaijibot-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/kaijibot-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/kaijibot-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/kaijibot-dev/kaijibot.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveKaijiBotAgentDir: mocks.resolveKaijiBotAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/kaijibot-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveKaijiBotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-6");
    expect(payload.configPath).toBe("/tmp/kaijibot-dev/kaijibot.json");
    expect(payload.auth.storePath).toBe("/tmp/kaijibot-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/kaijibot-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/kaijibot-agent-custom");
