...
function usesClaudeCliModelSelection(cfg: KaijiBotConfig): boolean {
...
function hasClaudeCliConfigSignals(cfg: KaijiBotConfig): boolean {
...
function resolveClaudeCliCommand(cfg: KaijiBotConfig): string {
...
function formatWorkspaceHealthLine(workspaceDir: string, health: ClaudeCliDirHealth): string {
  ...
  if (health === "missing") {
    return `- Workspace: ${display} (missing; KaijiBot will create it on first run).`;
  }
  ...
  if (credential) {
    ...
    fixHints.push(
      `- Fix: run ${formatCliCommand("claude auth login")}, then ${formatCliCommand(
        "kaijibot models auth login --provider anthropic --method cli --set-default",
      )}.`,
    );
  }

  if (!storedProfile) {
    lines.push(`- KaijiBot auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`);
    fixHints.push(
    ...
    lines.push(
      `- KaijiBot auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
    ...
    lines.push(
      `- KaijiBot auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`,
    );
  }
