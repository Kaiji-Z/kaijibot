...
import { type KaijiBotConfig, readConfigFileSnapshot } from "../../config/config.js";
...
export function formatGatewayChannelsStatusLines(payload: Record<string, unknown>): string[] {
  ...
  if (issues.length > 0) {
    ...
    lines.push(`- Run: ${formatCliCommand("kaijibot doctor")}`);
    lines.push("");
  }
  lines.push(
...
export async function formatConfigChannelsStatusLines(
  ...
  opts?: { sourceConfig?: KaijiBotConfig },
): Promise<string[]> {
  const lines: string[] = [];
