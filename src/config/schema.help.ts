...
export const FIELD_HELP: Record<string, string> = {
  ...
  cognitive:
    "认知系统是 KaijiBot 的主动 AI 层，包含用户画像、洞察生成、自进化和反馈收集四个子系统。启用后 Bot 会根据用户兴趣主动推送有价值的洞察，并从交互中学习进化。",
  "cognitive.enabled":
    "总开关。关闭后所有认知功能（主动推送、用户画像、自进化）全部停用，Bot 退回到纯被动模式。",
  "cognitive.proactive":
    "主动推送控制 Bot 何时、以什么频率向用户发送主动洞察。通过 PRISM 门控算法平衡推送价值和打扰程度。",
  "cognitive.proactive.enabled":
    "启用后 Bot 会在用户不活跃时主动推送有价值的跨领域洞察。关闭则完全不主动打扰用户。",
  "cognitive.proactive.minIntervalHours":
    "两次主动推送之间的最小间隔（小时）。实际间隔会在此基础上加入随机抖动（0.5x~1.5x），避免机械感。设小一点推送更频繁，设大一点更安静。",
  "cognitive.proactive.activeHours":
    "限制 Bot 只在指定时间段内主动推送。例如 start='09:00' end='22:00' 表示只在早9点到晚10点推送。",
  "cognitive.proactive.activeHours.start":
    "活跃时段开始时间（HH:MM 格式），如 '09:00'。",
  "cognitive.proactive.activeHours.end":
    "活跃时段结束时间（HH:MM 格式），如 '22:00'。",
  "cognitive.proactive.activeHours.timezone":
    "时区，如 'Asia/Shanghai'。留空则使用系统时区。",
  "cognitive.proactive.digestMode":
    "推送模式：realtime（每次洞察立即发送）、daily（每日汇总）、weekly（每周汇总）。",
  "cognitive.proactive.costFalseNegative":
    "漏推一条有价值的洞察的代价。值越大，系统越倾向于多推（宁可误推也不漏推）。",
  "cognitive.proactive.costFalseAlarm":
    "推送一条用户不感兴趣的洞察的代价。值越大，系统越保守（宁可少推也不误推）。",
  "cognitive.persona":
    "用户画像子系统：自动从对话中提取用户的兴趣领域、专业背景、交互偏好，用于个性化洞察生成。",
  "cognitive.persona.autoExtract":
    "启用后系统会自动从每次对话中提取用户画像信息（兴趣、领域、偏好）。关闭则需要手动维护。",
  "cognitive.persona.extractionModel":
    "用于画像提取的 LLM 模型。留空则使用默认模型。可以用更便宜的模型来降低成本。",
  "cognitive.persona.identityRefreshHours":
    "每隔多少小时重新刷新一次用户画像。频繁刷新更准确但消耗更多 token。",
  "cognitive.insight":
    "洞察生成子系统：基于用户画像和外部信息源生成有价值的跨领域洞察。",
  "cognitive.insight.sources":
    "洞察信息来源配置。控制从哪些渠道获取信息来生成洞察。",
  "cognitive.insight.sources.webSearchProvider":
    "网页搜索提供商（如 tavily、exa）。用于从互联网获取最新信息来丰富洞察内容。",
  "cognitive.insight.sources.scanIntervalHours":
    "每隔多少小时扫描一次外部信息源寻找洞察机会。",
  "cognitive.insight.sources.explicitTopics":
    "手动指定用户关注的主题列表。系统会优先围绕这些主题生成洞察。",
  "cognitive.insight.verificationLevel":
    "洞察验证严格程度：basic（基础验证）、strict（严格验证）、paranoid（偏执验证）。级别越高，生成的洞察越可靠但数量越少。",
  "cognitive.insight.inferenceModel":
    "用于洞察推断的 LLM 模型。留空则使用默认模型。可以用更强的模型来提升洞察质量。",
  "cognitive.insight.surpriseRatio":
    "意外推断比例（0-1）。0.8 表示 80% 的洞察朝未知方向推断，20% 基于已知兴趣。值越高洞察越有惊喜感但可能偏离用户需求。",
  "cognitive.insight.outputLanguage":
    "洞察输出语言。留空则自动匹配用户常用语言。可强制设为 'zh-CN'、'en' 等。",
  "cognitive.feedback":
    "反馈收集子系统：通过显式（表情/按钮）和隐式（回复速度、追问行为）方式收集用户对洞察的评价，用于优化推送策略。",
  "cognitive.feedback.mechanism":
    "用户反馈方式：emoji（用表情反应）、buttons（按钮）、text（文字反馈）。",
  "cognitive.feedback.implicitFeedback":
    "启用隐式反馈：系统自动从用户的回复行为中推断对洞察的兴趣程度（如是否追问、回复速度等）。",
  "cognitive.evolution":
    "自进化子系统：当 Bot 完成复杂任务或出错后，自动评估是否值得保存为可复用的 Skill，避免重复犯错。",
  "cognitive.evolution.enabled":
    "启用后系统会在任务出错或复杂任务完成时，建议用户将正确做法保存为 Skill。",
  "cognitive.evolution.minComplexity":
    "正常任务的最低复杂度阈值（0-1，默认 0.6）。只有超过此阈值的任务才会触发进化建议。",
  "cognitive.evolution.errorComplexityThreshold":
    "出错任务的降低阈值（0-1，默认 0.3）。当检测到工具调用错误或重试时使用此更低阈值，让容易出错的任务优先触发进化。",
  "cognitive.evolution.cooldownHours":
    "两次进化建议之间的最小间隔（小时）。防止短时间内频繁建议。",
  "cognitive.evolution.maxSuggestionsPerDay":
    "每天最多向用户提出多少次进化建议。",
  "cognitive.evolution.clawhubEnabled":
    "启用 ClawHub 技能分享平台。可以将生成的 Skill 分享给其他 KaijiBot 用户。",
  "cognitive.evolution.clawhubRegistry":
    "ClawHub 注册表 URL。默认为官方地址。",
  "cognitive.evolution.clawhubAutoPublish":
    "自动将用户采纳的 Skill 发布到 ClawHub。关闭则需要手动发布。",
};
