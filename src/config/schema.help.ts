...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by KaijiBot to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when KaijiBot writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for KaijiBot. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion":
    "KaijiBot version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping KaijiBot runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "agents.list[].runtime.type":
    'Runtime type for this agent: "embedded" (default KaijiBot runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this KaijiBot agent (for example codex, claude, cursor, gemini, kaijibot).",
  ...
  "browser.profiles.*.driver":
    'Per-profile browser driver mode. Use "kaijibot" (or legacy "clawd") for CDP-based profiles, or use "existing-session" for host-local Chrome DevTools MCP attachment.',
  ...
  "discovery.wideArea.domain":
    "Optional unicast DNS-SD domain for wide-area discovery, such as kaijibot.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
  "discovery.mdns":
  ...
  "agents.defaults.sandbox.browser.network":
    "Docker network for sandbox browser containers (default: kaijibot-sandbox-browser). Avoid bridge if you need stricter isolation.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /kaijibot).",
  ...
  "gateway.nodes.allowCommands":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `kaijibot security audit`.",
  ...
  "nodeHost.browserProxy.allowProfiles":
    "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, KaijiBot enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
  media:
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $KAIJIBOT_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "models.providers.*.injectNumCtxForOpenAICompat":
    "Controls whether KaijiBot injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
  ...
  "agents.defaults.memorySearch.store.path":
    "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.kaijibot/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
  ...
  "memory.backend":
    'Selects the global memory engine: "builtin" uses KaijiBot memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
  "memory.citations":
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `kaijibot plugins update` to locate install sources).",
  ...
  "agents.defaults.compaction.postCompactionSections":
    'AGENTS.md H2/H3 section names re-injected after compaction so the agent reruns critical startup guidance. Leave unset to use "Session Startup"/"Memory"/"Red Lines" with legacy fallback to "Every Session"/"Safety"; set to [] to disable reinjection entirely.',
  ...
  "agents.defaults.embeddedPi":
    "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in KaijiBot sessions.",
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage KaijiBot MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by KaijiBot. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings.",
  "mcp.servers":
    "Named MCP server definitions. KaijiBot stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  session:
  ...
  "session.parentForkMaxTokens":
    "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, KaijiBot starts a fresh thread session instead of forking; set 0 to disable this protection.",
  "session.mainKey":
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in KaijiBot. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
  ...
  "gateway.auth.token.id": "SecretRef provider id for gateway.auth.token. When the token is a SecretRef object, this identifies which provider resolves the secret.",
  "gateway.auth.token.provider": "SecretRef provider name for gateway.auth.token.",
  "gateway.auth.token.source": "SecretRef source type for gateway.auth.token (env, file, or exec).",
  "gateway.auth.password.id": "SecretRef provider id for gateway.auth.password.",
  "gateway.auth.password.provider": "SecretRef provider name for gateway.auth.password.",
  "gateway.auth.password.source": "SecretRef source type for gateway.auth.password (env, file, or exec).",
  "gateway.remote.token.id": "SecretRef provider id for gateway.remote.token.",
  "gateway.remote.token.provider": "SecretRef provider name for gateway.remote.token.",
  "gateway.remote.token.source": "SecretRef source type for gateway.remote.token.",
  "gateway.remote.password.id": "SecretRef provider id for gateway.remote.password.",
  "gateway.remote.password.provider": "SecretRef provider name for gateway.remote.password.",
  "gateway.remote.password.source": "SecretRef source type for gateway.remote.password.",
  "cron.webhookToken.id": "SecretRef provider id for cron.webhookToken.",
  "cron.webhookToken.provider": "SecretRef provider name for cron.webhookToken.",
  "cron.webhookToken.source": "SecretRef source type for cron.webhookToken.",
  "agents.defaults.blockStreamingChunk.breakPreference": 'Where to prefer splitting streamed block replies: "paragraph", "newline", or "sentence".',
  "agents.defaults.blockStreamingChunk.maxChars": "Maximum characters per streamed block chunk before splitting.",
  "agents.defaults.blockStreamingChunk.minChars": "Minimum characters per streamed block chunk.",
  "agents.defaults.blockStreamingCoalesce.idleMs": "Idle time in ms before flushing coalesced block replies.",
  "agents.defaults.blockStreamingCoalesce.maxChars": "Maximum characters to coalesce before flushing.",
  "agents.defaults.blockStreamingCoalesce.minChars": "Minimum characters to accumulate before flushing.",
  "agents.defaults.backgroundBatch": "Route background LLM pipelines (memory consolidation, correction extraction) through the provider Batch API for ~50% cost discount. Default: false.",
  "agents.defaults.contextPruning.mode": 'Context pruning mode: "off" (disabled) or "cache-ttl" (prune old tool results based on TTL).',
  "agents.defaults.contextPruning.ttl": "TTL duration for cache-ttl pruning mode (e.g. '30m', '2h'). Tool results older than this are pruned from context.",
  "agents.defaults.contextPruning.keepLastAssistants": "Number of recent assistant turns to always preserve from pruning.",
  "agents.defaults.contextPruning.softTrimRatio": "Fraction of original content kept during soft trimming (0-1).",
  "agents.defaults.contextPruning.hardClearRatio": "Fraction of original content kept during hard clearing (0-1). Lower = more aggressive pruning.",
  "agents.defaults.contextPruning.minPrunableToolChars": "Minimum tool result size in chars before it becomes eligible for pruning.",
  "agents.defaults.heartbeat.every": "Heartbeat interval as a duration string (e.g. '30m', '2h'). Controls how often the agent runs background heartbeat turns.",
  "agents.defaults.heartbeat.activeHours.start": "Start of the active hours window for heartbeat runs (24h HH:MM format). Heartbeats are suppressed before this time.",
  "agents.defaults.heartbeat.activeHours.end": "End of the active hours window for heartbeat runs (24h HH:MM format). Heartbeats are suppressed after this time.",
  "agents.defaults.heartbeat.activeHours.timezone": 'Timezone for heartbeat active hours. Accepts "user", "local", or IANA timezone string.',
  "agents.defaults.heartbeat.model": "Model override for heartbeat runs (provider/model). When unset, uses the agent default model.",
  "agents.defaults.heartbeat.session": "Session key for heartbeat runs. Default: 'main'.",
  "agents.defaults.heartbeat.to": "Delivery target for heartbeat output. Supports E.164 phone format or chat IDs.",
  "agents.defaults.heartbeat.accountId": "Account id override for multi-account heartbeat delivery.",
  "agents.defaults.heartbeat.target": 'Delivery channel for heartbeat: "last", "none", or a channel id.',
  "agents.defaults.heartbeat.prompt": "Custom prompt for heartbeat runs. Default reads HEARTBEAT.md from workspace.",
  "agents.defaults.heartbeat.ackMaxChars": "Max chars allowed after HEARTBEAT_OK before delivery (default: 300).",
  "agents.defaults.heartbeat.lightContext": "Run heartbeat with lightweight bootstrap context (only HEARTBEAT.md). Reduces token cost.",
  "agents.defaults.heartbeat.isolatedSession": "Run heartbeat in an isolated session with no prior conversation history. Dramatically reduces per-heartbeat token cost.",
  "agents.defaults.heartbeat.includeReasoning": "Deliver the model reasoning payload for heartbeat runs (default: false).",
  "agents.defaults.heartbeat.timeoutSeconds": "Run timeout in seconds for heartbeat agent turns.",
  "agents.defaults.subagents.model": "Default model for spawned sub-agents (string or {primary, fallbacks}).",
  "agents.defaults.sandbox.backend": 'Sandbox backend type: "docker", "ssh", or unset for auto-detection.',
  "agents.defaults.sandbox.mode": 'Sandbox session scoping: "off", "non-main", or "all". Controls which sessions run in sandbox.',
  "agents.defaults.sandbox.scope": "Session scope for sandbox: which conversations are eligible for sandboxed execution.",
  "agents.defaults.sandbox.sessionToolsVisibility": "Tool visibility within sandboxed sessions.",
  "agents.defaults.sandbox.workspaceAccess": 'Workspace file access level inside sandbox: "read-write", "read-only", or "none".',
  "agents.defaults.sandbox.workspaceRoot": "Root directory mounted into the sandbox as the workspace.",
  "messages.statusReactions.emojis.thinking": "Emoji for the thinking state (default: \u{1F914}).",
  "messages.statusReactions.emojis.tool": "Emoji for the tool-use state (default: \u{1F527}).",
  "messages.statusReactions.emojis.coding": "Emoji for the coding state (default: \u{1F4BB}).",
  "messages.statusReactions.emojis.web": "Emoji for the web-search state (default: \u{1F310}).",
  "messages.statusReactions.emojis.done": "Emoji for the completed state (default: \u2705).",
  "messages.statusReactions.emojis.error": "Emoji for the error state (default: \u274C).",
  "messages.statusReactions.emojis.stallSoft": "Emoji for soft stall warning (default: \u{1F7E1}).",
  "messages.statusReactions.emojis.stallHard": "Emoji for hard stall warning (default: \u{1F534}).",
  "messages.statusReactions.emojis.compacting": "Emoji for context compaction state (default: \u{1F9F9}).",
  "messages.statusReactions.timing.debounceMs": "Debounce interval for intermediate status states in ms (default: 700).",
  "messages.statusReactions.timing.stallSoftMs": "Soft stall warning timeout in ms (default: 25000).",
  "messages.statusReactions.timing.stallHardMs": "Hard stall warning timeout in ms (default: 60000).",
  "messages.statusReactions.timing.doneHoldMs": "How long to hold the done emoji before cleanup in ms (default: 1500).",
  "messages.statusReactions.timing.errorHoldMs": "How long to hold the error emoji before cleanup in ms (default: 2500).",
  "session.resetByType.direct.mode": 'Reset mode for direct chats: "daily" or "idle".',
  "session.resetByType.direct.atHour": "Daily reset hour (0-23) for direct chats.",
  "session.resetByType.direct.idleMinutes": "Idle reset window in minutes for direct chats.",
  "session.resetByType.group.mode": 'Reset mode for group chats: "daily" or "idle".',
  "session.resetByType.group.atHour": "Daily reset hour (0-23) for group chats.",
  "session.resetByType.group.idleMinutes": "Idle reset window in minutes for group chats.",
  "session.resetByType.thread.mode": 'Reset mode for thread chats: "daily" or "idle".',
  "session.resetByType.thread.atHour": "Daily reset hour (0-23) for thread chats.",
  "session.resetByType.thread.idleMinutes": "Idle reset window in minutes for thread chats.",
  "tools.web.search.apiKey": "API key for the web search provider. Prefer env/secret substitution.",
  "tools.web.x_search.enabled": "Enable X (Twitter) search tool using xAI Grok (default: true when xAI auth available).",
  "tools.web.x_search.model": "Model id for X search.",
  "tools.web.x_search.inlineCitations": "Keep inline citations in xAI response payload (default: true).",
  "tools.web.x_search.maxTurns": "Max search/tool turns for xAI internally.",
  "tools.web.x_search.timeoutSeconds": "Timeout for X search requests in seconds.",
  "tools.web.x_search.cacheTtlMinutes": "Cache TTL for X search results in minutes.",
  "tools.sandbox.tools.allow": "Tool allowlist for sandboxed agent executions.",
  "tools.sandbox.tools.alsoAllow": "Additional tools allowed in sandbox beyond defaults.",
  "tools.sandbox.tools.deny": "Tools denied in sandboxed executions.",
  "tools.subagents.tools.allow": "Tool allowlist for spawned sub-agents.",
  "tools.subagents.tools.alsoAllow": "Additional tools allowed for sub-agents beyond defaults.",
  "tools.subagents.tools.deny": "Tools denied for sub-agents.",
  "tools.sessions_spawn.attachments.enabled": "Enable file attachments in sub-agent spawn requests.",
  "tools.sessions_spawn.attachments.maxFileBytes": "Maximum file size in bytes for spawn attachments.",
  "tools.sessions_spawn.attachments.maxFiles": "Maximum number of files per spawn request.",
  "cron.failureAlert.enabled": "Enable failure alert notifications for cron jobs.",
  "cron.failureAlert.mode": "Alert delivery mode: 'session' or 'targets'.",
  "cron.failureAlert.after": "Number of failed attempts before alerting.",
  "cron.failureAlert.cooldownMs": "Cooldown in ms between repeated failure alerts.",
  "cron.failureAlert.accountId": "Account id for multi-account alert delivery.",
  "cron.failureDestination.channel": "Channel id for cron failure notifications.",
  "cron.failureDestination.to": "Destination identifier for failure notifications.",
  "cron.failureDestination.mode": "Delivery mode for failure notifications.",
  "cron.failureDestination.accountId": "Account id for failure notification delivery.",
  "tools.media.image.baseUrl": "Base URL override for image understanding provider requests. See docs/nodes/media-understanding.md for details.",
  "tools.media.image.echoFormat": "Format string for echoed audio transcripts. Use {transcript} as placeholder.",
  "tools.media.image.echoTranscript": "Echo the transcript back to chat before agent processing so users can verify what was heard.",
  "tools.media.video.baseUrl": "Base URL override for video understanding provider requests.",
  "tools.media.audio.baseUrl": "Base URL override for audio understanding provider requests.",
  "agents.defaults.memorySearch.store.fts.tokenizer": "FTS5 tokenizer: 'porter' (default), 'unicode61', or 'trigram'.",
};
