...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): KaijiBotConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: KaijiBotConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        KAIJIBOT_STATE_DIR: envStateDir,
      };
      const cfg: KaijiBotConfig = {};
