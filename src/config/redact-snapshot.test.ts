...
import type { ConfigFileSnapshot } from "./types.kaijibot.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/kaijibot/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/kaijibot/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/kaijibot/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/kaijibot/nickserv-password.txt");
