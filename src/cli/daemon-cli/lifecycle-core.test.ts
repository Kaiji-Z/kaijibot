...
const loadConfig = vi.fn<() => KaijiBotConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { KAIJIBOT_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("KAIJIBOT_CONTAINER_HINT", "kaijibot-demo-container");
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for kaijibot-demo-container.",
    );
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { KAIJIBOT_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("KAIJIBOT_GATEWAY_TOKEN", "env-token");
  ...
  it("resolves config token SecretRefs using service command env before drift checks", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: {
        KAIJIBOT_GATEWAY_TOKEN: "service-token",
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    await runServiceStart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => ["kaijibot gateway install"],
      opts: { json: true },
    });
    ...
    expect(payload.hints).toEqual(expect.arrayContaining(["kaijibot gateway install"]));
    expect(payload.hintItems).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "install",
          text: "kaijibot gateway install",
        }),
      ]),
    );
    expect(service.restart).not.toHaveBeenCalled();
