...
const serveKaijiBotChannelMcp = mocks.serveKaijiBotChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveKaijiBotChannelMcp: mocks.serveKaijiBotChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kaijibot-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("kaijibot-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("kaijibot-cli-mcp-home-", async () => {
      ...
      expect(serveKaijiBotChannelMcp).toHaveBeenCalledWith({
