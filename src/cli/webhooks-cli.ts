...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "gitee.com/kaiji1126/kaijibot/blob/main/docs/cli/webhooks.md")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + KaijiBot hooks")
    ...
    .option("--hook-url <url>", "KaijiBot hook URL")
    .option("--hook-token <token>", "KaijiBot hook token")
