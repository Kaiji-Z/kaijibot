...
function makePersona(): PersonaTree {
  ...
  return {
    ...
    domains: {
      ...
      "TypeScript": { depth: 5, recurrence: 20, lastMentioned: now - 1000 * 60 * 10, keyInsights: ["decorator pattern", "Zod schema validation"], activeQuestions: [], negationSignals: 0 },
      "MCP": { depth: 3, recurrence: 5, lastMentioned: now - 1000 * 60 * 60, keyInsights: ["Model Context Protocol", "tool schema design"], activeQuestions: [], negationSignals: 0 },
      "Rust": { depth: 4, recurrence: 8, lastMentioned: now - 1000 * 60 * 45, keyInsights: ["borrow checker", "zero-cost abstractions"], activeQuestions: [], negationSignals: 0 },
