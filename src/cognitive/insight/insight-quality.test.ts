...
function evaluateQuality(
  ...
  if (!mentionedDomain && !mentionedFocus) {
    issues.push("未提及用户领域或近期关注");
  }
  ...
  if (bannedHits > 0) {
    issues.push(`检测到 ${bannedHits} 个模板句式`);
  }
  ...
  if (!usesKeyInsight) {
    issues.push("未引用用户的具体 keyInsight");
  }
