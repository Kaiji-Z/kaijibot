...
export class FragmentStore {
  ...
  async load(agentId: string, userId: string): Promise<Fragment[]> {
    ...
    try {
      ...
      this.cache.set(key, {
        fragments: (parsed as FragmentStoreFile).fragments,
        loadedAt: Date.now(),
      });
      return fragments;
    } catch (err) {
  ...
  async findClusters(agentId: string, userId: string): Promise<FragmentCluster[]> {
    ...
    for (const [_root, ids] of groups) {
      ...
      if (groupFragments.length < 2) {
        continue;
      }
      if (allDomains.size < 2 && tensionCount < 1 && groupFragments.length < 3) {
        continue;
      }
      if (avgStrength < 0.15) {
        continue;
    ...
    clusters.sort(
      (a, b) => b.averageStrength * b.fragmentIds.length - a.averageStrength * a.fragmentIds.length,
    );
    log.info("fragment clusters", {
      agentId,
      userId,
      clusterCount: clusters.length,
      sizes: clusters.map((c) => c.fragmentIds.length),
      domains: clusters.map((c) => [...c.domains]),
    });
    return clusters;
  }
