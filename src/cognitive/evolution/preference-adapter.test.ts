...
describe("EvolutionPreferenceAdapter", () => {
  it("accept increases alpha for domain", async () => {
    await adapter.recordResponse(AGENT, "user-1", "feishu-wiki", "accepted");
  ...
  it("reject increases beta for domain", async () => {
    await adapter.recordResponse(AGENT, "user-1", "code-review", "rejected");
  ...
  it("modified counts as partial acceptance (alpha + 0.5)", async () => {
    await adapter.recordResponse(AGENT, "user-1", "analytics", "modified");
  ...
  it("domain with no history returns prior (~0.67)", async () => {
    const rate = await adapter.getDomainAcceptanceRate(AGENT, "user-1", "unknown-domain");
  ...
  it("domain with all rejections returns low score", async () => {
    ...
    for (let i = 0; i < 20; i++) {
      await adapter.recordResponse(AGENT, "user-1", "bad-domain", "rejected");
    }
    const rate = await adapter.getDomainAcceptanceRate(AGENT, "user-1", "bad-domain");
  ...
  it("getTopDomains returns sorted by sampled score", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      await adapter.recordResponse(AGENT, "user-1", "good-domain", "accepted");
    }
    for (let i = 0; i < 10; i++) {
      await adapter.recordResponse(AGENT, "user-1", "bad-domain", "rejected");
    }
    const top = await adapter.getTopDomains(AGENT, "user-1", 2);
  ...
  it("persists across instances (write then new adapter reads)", async () => {
    ...
    const bandit = await adapter2.getRawBandit(AGENT, "user-1", "feishu-wiki");
    expect(bandit).toBeDefined();
  ...
  it("isolates data between different agents", async () => {
    await adapter.recordResponse("agent-a", "user-1", "feishu-wiki", "accepted");

    const banditB = await adapter.getRawBandit("agent-b", "user-1", "feishu-wiki");
    expect(banditB).toBeUndefined();

    const banditA = await adapter.getRawBandit("agent-a", "user-1", "feishu-wiki");
    expect(banditA).toBeDefined();
    expect(banditA!.alpha).toBe(3);
  });
});
