...
import { FeishuActivitySource } from "./feishu-activity-source.js";

describe("FeishuActivitySource", () => {
  afterEach(() => {
    vi.useRealTimers();
  });

  it("constructs without error", () => {
    const source = new FeishuActivitySource(5000);
    expect(source).toBeInstanceOf(FeishuActivitySource);
  });

  it("emits events at configured interval", () => {
    vi.useFakeTimers();
    const source = new FeishuActivitySource(1000);
  ...
  it("emits events with type activity_scan and payload", () => {
    vi.useFakeTimers();
    const source = new FeishuActivitySource(1000);
    ...
    expect(events[0].type).toBe("activity_scan");
  ...
  it("delivers events to all registered listeners", () => {
    vi.useFakeTimers();
    const source = new FeishuActivitySource(500);
