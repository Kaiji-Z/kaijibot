...
describe("local media roots", () => {
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "kaijibot-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "kaijibot-agent-media-roots-state"),
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "kaijibot-flexible-media-roots-state"),
      cfg: {},
      shouldContainPictures: false,
    },
    {
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "kaijibot-messaging-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "kaijibot-messaging-fs-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
  ...
  it("keeps agent-scoped defaults even when mediaSources include file URLs and top-level paths", () => {
    const stateDir = path.join("/tmp", "kaijibot-file-url-media-roots-state");
    const picturesDir =
  ...
  it("includes the config media root when legacy state and config dirs diverge", () => {
    const homeRoot = path.join(os.tmpdir(), "kaijibot-legacy-home-test");
    const roots = buildMediaLocalRoots(
      path.join(homeRoot, ".clawdbot"),
      path.join(homeRoot, ".kaijibot"),
    );
    ...
    expectNormalizedRootsContain(roots, [
      ...
      path.join(homeRoot, ".kaijibot", "media"),
    ]);
  });
});
