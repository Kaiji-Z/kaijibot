...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `kaijibot-${params.binName}-bin-`,
    run: params.run,
  });
}
...
function withFakeRuntimeBins<T>(params: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), params.tmpPrefix ?? "kaijibot-runtime-bins-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "kaijibot-ruby-require-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "kaijibot-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "kaijibot-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "kaijibot-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "kaijibot-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "kaijibot-inline-shell-node-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "kaijibot-pnpm-dlx-unknown-flag-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "kaijibot-pnpm-dlx-unknown-prefix-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "kaijibot-pnpm-dlx-unknown-prefix-value-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "kaijibot-pnpm-dlx-global-double-dash-",
    command: ["pnpm", "--", "dlx", "--future-flag", "tsx", "./run.ts"],
    setup: (tmp) => {
      fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE")\n');
    },
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  it.runIf(process.platform !== "win32").each(cases)("$name", (testCase) => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "kaijibot-approval-hardening-"));
    const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "kaijibot-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("detects rewritten script operands for pnpm dlx approval plans", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "kaijibot-pnpm-dlx-approval-",
            fixture: {
  ...
  it("does not bind pnpm dlx shell-mode commands to a mutable file operand", () => {
    withFakeRuntimeBins({
      binNames: ["pnpm", "tsx"],
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "kaijibot-pnpm-dlx-shell-mode-"));
        try {
          fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE");\n');
          expect(
            resolveMutableFileOperandSnapshotSync({
  ...
  it("allows pnpm dlx package binaries that do not bind a mutable local file", () => {
    withFakeRuntimeBin({
      binName: "pnpm",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "kaijibot-pnpm-dlx-package-bin-"));
        try {
    ...
    withFakeRuntimeBin({
      ...
      run: () => {
        const tmp = fs.mkdtempSync(
          path.join(os.tmpdir(), "kaijibot-pnpm-dlx-package-runtime-token-"),
        );
        try {
  ...
  it("allows pnpm dlx package binaries with multi-token data-like runtime names", () => {
    withFakeRuntimeBin({
      ...
      run: () => {
        const tmp = fs.mkdtempSync(
          path.join(os.tmpdir(), "kaijibot-pnpm-dlx-package-runtime-token-multi-"),
        );
        try {
  ...
  it("allows pnpm dlx package binaries with local file arguments", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "kaijibot-pnpm-dlx-package-file-"));
        try {
  ...
  it("allows pnpm dlx package binaries with interpreter-like data tails", () => {
    withFakeRuntimeBin({
      binName: "pnpm",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "kaijibot-pnpm-dlx-package-data-tail-"));
        try {
          fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE");\n');
  ...
  it("treats -- as the end of pnpm dlx option parsing", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "kaijibot-pnpm-dlx-double-dash-",
            fixture: {
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "kaijibot-shell-option-value-"));
    try {
