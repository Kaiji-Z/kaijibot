...
function stubSlackPlugin(params: {
  resolveAccount: (cfg: KaijiBotConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: KaijiBotConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: KaijiBotConfig) => boolean;
}): ChannelPlugin {
  return {
    id: "slack",
    meta: {
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: KaijiBotConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {
