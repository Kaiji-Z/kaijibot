...
describe("bundled channel entry shape guards", () => {
  ...
  it("does not include dead extension directories in the bundled set", () => {
    const bundledIds = loadPluginManifestRegistry({ cache: true, config: {} })
      .plugins.filter((plugin) => plugin.origin === "bundled")
      .map((plugin) => plugin.id);

    for (const deadId of DEAD_EXTENSION_IDS) {
      expect(bundledIds, `dead extension "${deadId}" should not be bundled`).not.toContain(deadId);
    }
  });

  it("feishu extension is present and has valid shape", () => {
    const feishuRoot = bundledPluginRoots.find((dir) => dir.includes("feishu"));
  ...
  it("loads real bundled channel entries from the source tree", async () => {
    ...
    expect(bundled.requireBundledChannelPlugin("feishu").id).toBe("feishu");
    expect(() =>
      bundled.setBundledChannelRuntime("feishu", {
        channel: {
          feishu: {},
        },
      } as never),
  ...
  it("keeps channel entrypoints on the dedicated entry-contract SDK surface", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      for (const relativePath of ["index.ts", "channel-entry.ts", "setup-entry.ts"]) {
        ...
        if (
          !source.includes('from "kaijibot/plugin-sdk/channel-entry-contract"') ||
          source.includes('from "kaijibot/plugin-sdk/core"') ||
          source.includes('from "kaijibot/plugin-sdk/channel-core"')
        ) {
  ...
  it("keeps channel implementations off the broad core SDK surface", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      for (const relativePath of ["src/channel.ts", "src/plugin.ts"]) {
        ...
        if (source.includes('from "kaijibot/plugin-sdk/core"')) {
          offenders.push(path.relative(process.cwd(), filePath));
        }
      }
    }
  ...
  it("breaks reentrant bundled channel discovery cycles with an empty fallback", async () => {
    const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "kaijibot-bundled-reentrant-"));
