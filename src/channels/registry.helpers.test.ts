...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://kaijibot.ai");
  });
  ...
  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    ...
    startupRegistry.channels = [
      {
        pluginId: "kaijibot-weixin",
        plugin: { id: "kaijibot-weixin", meta: { aliases: ["weixin"] } },
        source: "test",
      },
    ] as never;
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["kaijibot-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("kaijibot-weixin");
  });
