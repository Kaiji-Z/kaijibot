...
summary: "How KaijiBot remembers things across sessions"
read_when:
...
KaijiBot remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.kaijibot/workspace`).
...
<Info>
KaijiBot auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, KaijiBot
...
```bash
kaijibot memory rem-backfill --rollback
kaijibot memory rem-backfill --rollback-short-term
```

## CLI

```bash
kaijibot memory status          # Check index status and provider
kaijibot memory search "query"  # Search from the command line
kaijibot memory index --force   # Rebuild the index
```
