...
- `/usage cost` in chats: local cost summary aggregated from KaijiBot session logs.
- CLI: `kaijibot status --usage` prints a full per-provider breakdown.
- CLI: `kaijibot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. KaijiBot treats
  ...
  quota, so KaijiBot inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, KaijiBot prefers the
...
can supply plugin-specific usage auth logic; otherwise KaijiBot falls back to
