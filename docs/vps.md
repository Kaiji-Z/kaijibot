---
summary: "Run KaijiBot on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for KaijiBot
title: "Linux Server"
...
Run the KaijiBot Gateway on any Linux server or cloud VPS. This page helps you
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/kaijibot-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `KAIJIBOT_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  - `KAIJIBOT_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/kaijibot-compile-cache`
...
For the standard `kaijibot onboard --install-daemon` path, edit the user unit:
...
`kaijibot-gateway.service` via `sudo systemctl edit kaijibot-gateway.service`.
