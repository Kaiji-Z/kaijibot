...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.kaijibot/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so KaijiBot picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    kaijibot agent --message "give me a greeting"
    ```
...
| `metadata.kaijibot.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.kaijibot.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.kaijibot.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `kaijibot agent --message "..."` to test before sharing
...
| `~/.kaijibot/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with KaijiBot) | Low        | Global                |
