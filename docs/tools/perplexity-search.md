...
KaijiBot supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, KaijiBot also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `kaijibot configure --section web`. It stores the key in
`~/.kaijibot/kaijibot.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.kaijibot/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
