...
`ask: "always"` in `~/.kaijibot/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `kaijibot approvals get`, `kaijibot approvals get --gateway`, or
`kaijibot approvals get --node <id|name|ip>` to inspect the requested policy,
...
- **gateway host** → `kaijibot` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, KaijiBot also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.kaijibot/exec-approvals.sock",
...
- requested exec policy in KaijiBot config (`tools.exec.*`)
- host-local approvals policy in `~/.kaijibot/exec-approvals.json`
...
- In YOLO mode, KaijiBot does not add a separate heuristic command-obfuscation approval gate on top of the configured host exec policy.
...
```bash
kaijibot approvals set --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
```bash
kaijibot approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
When `tools.exec.strictInlineEval=true`, KaijiBot treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.kaijibot/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `kaijibot approvals allowlist ...`).
- `kaijibot security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `kaijibot doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, KaijiBot still rejects the `env` builtin in safe-bin
...
`~/.kaijibot/exec-approvals.json` directly.

CLI: `kaijibot approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, KaijiBot sends a followup `agent` turn to the same session.
...
topics, KaijiBot preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, KaijiBot prevents the agent from reusing
