...
KaijiBot supports Grok as a `web_search` provider, using xAI web-grounded
...
`plugins.entries.xai.config.webSearch.apiKey`, KaijiBot now reuses it as a
...
KaijiBot can show a separate follow-up step to enable `x_search` with the same
