...
read_when:
  - Running KaijiBot Gateway in WSL2 while Chrome lives on Windows
...
- KaijiBot Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/kaijibot/kaijibot/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- KaijiBot and Chrome are on the same machine
...
- KaijiBot points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, KaijiBot is not the problem yet.
...
Fix that before touching KaijiBot config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point KaijiBot at the address that is reachable from WSL2:
...
- use HTTP(S) when you want KaijiBot to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting KaijiBot to succeed
...
```bash
kaijibot browser open https://example.com --browser-profile remote
kaijibot browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `kaijibot browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `kaijibot browser stop --browser-profile remote`
...
3. KaijiBot config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug KaijiBot config or Control UI auth
