...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `kaijibot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `kaijibot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=kaijibot-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${KAIJIBOT_STATE_DIR:-$HOME/.kaijibot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
