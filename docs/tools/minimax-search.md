...
KaijiBot supports MiniMax as a `web_search` provider through the MiniMax
...
KaijiBot also accepts `MINIMAX_CODING_API_KEY` as an env alias. `MINIMAX_API_KEY`
...
If `plugins.entries.minimax.config.webSearch.region` is unset, KaijiBot resolves
...
- `query`
- `count` (KaijiBot trims the returned result list to the requested count)
