...
KaijiBot also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Configure">
    ```bash
    kaijibot configure --section web
    ```
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "KaijiBot plugin SDK" });
    ```
...
Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of KaijiBot's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, KaijiBot keeps the normal managed `web_search` behavior.
...
If no `provider` is set, KaijiBot checks providers in this order and uses the
...
<Note>
  ...
  KaijiBot resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
- or omit that field and let KaijiBot auto-detect the first ready web-fetch
  provider from available credentials
- today the bundled web-fetch provider is Firecrawl, configured under
  `plugins.entries.firecrawl.config.webFetch.*`

When you choose **Kimi** during `kaijibot onboard` or
`kaijibot configure --section web`, KaijiBot can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `kaijibot doctor --fix`.
When you choose Grok during `kaijibot onboard` or `kaijibot configure --section web`,
KaijiBot can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, KaijiBot does not
...
<Tabs>
  <Tab title="Config file">
    Run `kaijibot configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.kaijibot/.env`.
...
optional structured filters. KaijiBot only enables the built-in xAI `x_search`
