...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in KaijiBot
...
Running KaijiBot via Node with `tsx` fails at startup with:

```
[kaijibot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `kaijibot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch kaijibot.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node kaijibot.mjs status` works on Node 25.
