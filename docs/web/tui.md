...
```bash
kaijibot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `kaijibot tui --deliver`
...
- Local shell commands receive `KAIJIBOT_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `KAIJIBOT_THEME=light` before launching `kaijibot tui`.
- To force the original dark palette instead, set `KAIJIBOT_THEME=dark`.
...
- Check the Gateway logs: `kaijibot logs --follow`.
- Confirm the agent can run: `kaijibot status` and `kaijibot models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `kaijibot agents list` and your routing config.
