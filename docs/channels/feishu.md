...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects KaijiBot to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current KaijiBot releases, so no separate plugin install
is required.
...
```bash
kaijibot plugins install @kaijibot/feishu
```
...
If you just installed KaijiBot, run onboarding:
...
2. Configuring app credentials in KaijiBot
...
1. You already ran `kaijibot channels add` for Feishu
2. The gateway is running (`kaijibot gateway status`)
...
```bash
kaijibot pairing approve feishu <CODE>
```
...
2. Run `kaijibot logs --follow` and look for `chat_id`
...
2. Run `kaijibot logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `kaijibot gateway status`  | Show gateway status           |
| `kaijibot gateway install` | Install/start gateway service |
| `kaijibot gateway stop`    | Stop gateway service          |
| `kaijibot gateway restart` | Restart gateway service       |
| `kaijibot logs --follow`   | Tail gateway logs             |
...
4. Check logs: `kaijibot logs --follow`
...
5. Ensure the gateway is running: `kaijibot gateway status`
6. Check logs: `kaijibot logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.kaijibot/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.kaijibot/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
