...
Microsoft Teams ships as a bundled plugin in current KaijiBot releases, so no
...
```bash
kaijibot plugins install @kaijibot/msteams
```

Local checkout (when running from a git repo):

```bash
kaijibot plugins install ./path/to/local/msteams-plugin
```
...
3. Configure KaijiBot with those credentials.
...
- Talk to KaijiBot via Teams DMs, group chats, or channels.
...
- On startup, KaijiBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.kaijibot/kaijibot.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring KaijiBot, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `kaijibot-msteams` (must be unique) |
...
1. **Ensure the Microsoft Teams plugin is available**
   ...
   - Older/custom installs can add it manually:
     - From npm: `kaijibot plugins install @kaijibot/msteams`
     - From a local checkout: `kaijibot plugins install ./path/to/local/msteams-plugin`
...
KaijiBot exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "KaijiBot in Teams", full: "KaijiBot in Teams" },
...
KaijiBot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, KaijiBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/KaijiBotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

KaijiBot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `kaijibot message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.kaijibot/msteams-polls.json`.
...
```bash
kaijibot message send --channel msteams \
...
kaijibot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
kaijibot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
kaijibot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
kaijibot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
