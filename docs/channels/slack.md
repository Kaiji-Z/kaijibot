...
```json
{
  "display_information": {
    "name": "KaijiBot",
    "description": "Slack connector for KaijiBot"
  },
  "features": {
    "bot_user": {
      "display_name": "KaijiBot",
      "always_online": true
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/kaijibot",
        "description": "Send a message to KaijiBot",
        "should_escape": false
      }
    ]
  },
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `kaijibot pairing approve slack <code>`.
...
- If streaming fails mid-reply, KaijiBot falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while KaijiBot is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
- The interactive callback values are KaijiBot-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, KaijiBot falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `kaijibot channels status --probe --json` shows `botTokenStatus` or
