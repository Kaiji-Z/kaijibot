...
Nextcloud Talk ships as a bundled plugin in current KaijiBot releases, so
...
```bash
kaijibot plugins install @kaijibot/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
kaijibot plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "KaijiBot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `kaijibot pairing list nextcloud-talk`
  - `kaijibot pairing approve nextcloud-talk <CODE>`
