---
summary: "Synology Chat webhook setup and KaijiBot config"
read_when:
  - Setting up Synology Chat with KaijiBot
...
Synology Chat ships as a bundled plugin in current KaijiBot releases, so normal
...
```bash
kaijibot plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `kaijibot onboard` now shows Synology Chat in the same channel setup list as `kaijibot channels add`.
   - Non-interactive setup: `kaijibot channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your KaijiBot gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in KaijiBot.
   - Guided: `kaijibot onboard`
   - Direct: `kaijibot channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- KaijiBot accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `kaijibot pairing list synology-chat`
  - `kaijibot pairing approve synology-chat <CODE>`
...
```bash
kaijibot message send --channel synology-chat --target 123456 --text "Hello from KaijiBot"
kaijibot message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. KaijiBot now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached KaijiBot
- `Rate limit exceeded`:
