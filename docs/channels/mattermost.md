---
summary: "Mattermost bot setup and KaijiBot config"
read_when:
...
Mattermost ships as a bundled plugin in current KaijiBot releases, so normal
...
```bash
kaijibot plugins install @kaijibot/mattermost
```

Local checkout (when running from a git repo):

```bash
kaijibot plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure KaijiBot and start the gateway.
...
Native slash commands are opt-in. When enabled, KaijiBot registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, KaijiBot derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when KaijiBot registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as KaijiBot.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to KaijiBot.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from KaijiBot, not `404`.
...
- Approve via:
  - `kaijibot pairing list mattermost`
  - `kaijibot pairing approve mattermost <CODE>`
...
Use these target formats with `kaijibot message send` or cron/webhooks:
...
KaijiBot resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), KaijiBot sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When KaijiBot sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, KaijiBot derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and KaijiBot run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="kaijibot-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"kaijibot-mattermost-interactions",
...
`kaijibot message send` and cron/webhook deliveries.
...
- Native slash commands return `Unauthorized: invalid command token.`: KaijiBot
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as KaijiBot. Set an
