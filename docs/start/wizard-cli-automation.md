---
summary: "Scripted onboarding and agent setup for the KaijiBot CLI"
read_when:
...
Use `--non-interactive` to automate `kaijibot onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but KaijiBot now prefers Claude CLI reuse when available.
