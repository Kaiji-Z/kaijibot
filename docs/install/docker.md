---
summary: "Optional Docker-based setup and onboarding for KaijiBot"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run KaijiBot on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export KAIJIBOT_IMAGE="ghcr.io/kaijibot/kaijibot:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/kaijibot/kaijibot/pkgs/container/kaijibot).
  ...
  <Step title="Complete onboarding">
    ...
    `kaijibot-gateway` directly. `kaijibot-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm kaijibot-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm kaijibot-cli channels login

    # Telegram
    docker compose run --rm kaijibot-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm kaijibot-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t kaijibot:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node kaijibot-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node kaijibot-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d kaijibot-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `KAIJIBOT_EXTRA_MOUNTS`
or `KAIJIBOT_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `kaijibot-cli` shares `kaijibot-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d kaijibot-gateway`, run onboarding
and setup-time config writes through `kaijibot-gateway` with
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `KAIJIBOT_IMAGE`               | Use a remote image instead of building locally                   |
| `KAIJIBOT_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `KAIJIBOT_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `KAIJIBOT_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `KAIJIBOT_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `KAIJIBOT_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `KAIJIBOT_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
```bash
docker compose exec kaijibot-gateway node dist/index.js health --token "$KAIJIBOT_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `KAIJIBOT_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `KAIJIBOT_CONFIG_DIR` to `/home/node/.kaijibot` and
`KAIJIBOT_WORKSPACE_DIR` to `/home/node/.kaijibot/workspace`, so those paths
survive container replacement.

That mounted config directory is where KaijiBot keeps:

- `kaijibot.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `KAIJIBOT_GATEWAY_TOKEN`

For full persistence details on VM deployments, see
[Docker VM Runtime - What persists where](/install/docker-vm-runtime#what-persists-where).

**Disk growth hotspots:** watch `media/`, session JSONL files, `cron/runs/*.jsonl`,
and rolling file logs under `/tmp/kaijibot/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/kaijibot/kaijibot/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export KAIJIBOT_SANDBOX=1
    export KAIJIBOT_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm kaijibot-cli gateway probe
    docker compose run -T --rm kaijibot-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `kaijibot-cli` uses `network_mode: "service:kaijibot-gateway"` so CLI
    ...
    `no-new-privileges` on `kaijibot-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.kaijibot`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/kaijibot-config /path/to/kaijibot-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export KAIJIBOT_HOME_VOLUME="kaijibot_home"`
    2. **Bake system deps**: `export KAIJIBOT_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `KAIJIBOT_HOME_VOLUME` or `KAIJIBOT_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/kaijibot/kaijibot/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    KaijiBot runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm kaijibot-cli dashboard --no-open
    docker compose run --rm kaijibot-cli devices list
    docker compose run --rm kaijibot-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm kaijibot-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm kaijibot-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping KaijiBot up to date
