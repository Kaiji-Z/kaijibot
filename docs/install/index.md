---
summary: "Install KaijiBot — installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs KaijiBot, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://kaijibot.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want KaijiBot and Node kept under a local prefix such as
`~/.kaijibot`, without depending on a system-wide Node install:

```bash
curl -fsSL https://kaijibot.ai/install-cli.sh | bash
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g kaijibot@latest
```
...
Or skip the link and use `pnpm kaijibot ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:kaijibot/kaijibot#main
```
...
- macOS: LaunchAgent via `kaijibot onboard --install-daemon` or `kaijibot gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy KaijiBot on a cloud server or VPS:
...
If the install succeeded but `kaijibot` is not found in your terminal:
