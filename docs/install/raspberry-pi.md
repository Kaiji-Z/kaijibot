---
summary: "Host KaijiBot on a Raspberry Pi for always-on self-hosting"
read_when:
...
Run a persistent, always-on KaijiBot Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well.
...
**Service will not start** -- Check logs with `journalctl --user -u kaijibot-gateway.service --no-pager -n 100` and run `kaijibot doctor --non-interactive`. If this is a headless Pi, also verify lingering is enabled: `sudo loginctl enable-linger "$(whoami)"`.
