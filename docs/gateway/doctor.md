...
`kaijibot doctor` is the repair + migration tool for KaijiBot. It fixes stale
...
```bash
kaijibot doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
kaijibot doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
kaijibot doctor --non-interactive
```
...
- Extra workspace dir detection (`~/kaijibot`).
...
doctor-style RPC methods, but they are **not** part of `kaijibot doctor` CLI
repair/migration.
...
you to run `kaijibot doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.kaijibot/kaijibot.json` with the updated schema.

The Gateway also auto-runs doctor migrations on startup when it detects a
legacy config format, so stale configs are repaired without manual intervention.
Cron job store migrations are handled by `kaijibot doctor --fix`.
...
- Sessions store + transcripts:
  - from `~/.kaijibot/sessions/` to `~/.kaijibot/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.kaijibot/agent/` to `~/.kaijibot/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.kaijibot/credentials/*.json` (except `oauth.json`)
  - to `~/.kaijibot/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `kaijibot doctor`. Talk provider/provider-map normalization now
...
Doctor also checks the cron job store (`~/.kaijibot/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.kaijibot` folders exist across
  home directories or when `KAIJIBOT_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.kaijibot/kaijibot.json` is
...
that re-auth is required and prints the exact `kaijibot models auth login --provider ...`
command to run.
...
Discord plugin runtime packages) are present in the KaijiBot install root.
If any are missing, doctor reports the packages and installs them in
`kaijibot doctor --fix` / `kaijibot doctor --repair` mode.

### 8) Gateway service migrations and cleanup hints

Doctor detects legacy gateway services (launchd/systemd/schtasks) and
offers to remove them and install the KaijiBot service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named KaijiBot gateway services are considered first-class and are not
flagged as "extra."
...
startup continues. In read-only mode (`kaijibot doctor` without `--fix`) this check
is skipped entirely.
...
- **Legacy workspace dirs**: warns when `~/kaijibot` or other legacy workspace directories
...
- If the shell profile uses a slow dynamic completion pattern
  (`source <(kaijibot completion ...)`), doctor upgrades it to the faster
  cached file variant.
...
Run `kaijibot completion --write-state` to regenerate the cache manually.
...
- `kaijibot doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `kaijibot doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
Use `kaijibot memory status --deep` to verify embedding readiness at runtime.
...
- `kaijibot doctor` prompts before rewriting supervisor config.
- `kaijibot doctor --yes` accepts the default repair prompts.
- `kaijibot doctor --repair` applies recommended fixes without prompts.
- `kaijibot doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `kaijibot gateway install --force`.
