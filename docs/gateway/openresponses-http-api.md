...
KaijiBot’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`kaijibot agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-kaijibot-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-kaijibot-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "kaijibot"`, `model: "kaijibot/default"`, `model: "kaijibot/<agentId>"`, or `x-kaijibot-agent-id`
- use `x-kaijibot-model` when you want to override the selected agent's backend model
- use `x-kaijibot-session-key` for explicit session routing
- use `x-kaijibot-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: KaijiBot reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
KaijiBot normalizes common OpenAI-style aliases before those counters reach
