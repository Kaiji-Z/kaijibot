...
KaijiBot supports OAuth and API keys for model providers. For always-on gateway
...
setup, but KaijiBot also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `kaijibot gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.kaijibot/.env` so the daemon can read it:

```bash
cat >> ~/.kaijibot/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `kaijibot onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.kaijibot/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in KaijiBot as a supported token
path. Anthropic staff has since told us that KaijiBot-style Claude CLI usage is
...
```bash
kaijibot models auth paste-token --provider openrouter
```
...
- If auth exists but KaijiBot cannot resolve a probeable model candidate for
...
- Anthropic staff told us this KaijiBot integration path is allowed again.
- KaijiBot therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `KAIJIBOT_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- KaijiBot retries with the next key only for rate-limit errors (for example
...
When you debug order issues, `kaijibot models status --probe` shows omitted
...
Run `kaijibot models status` to confirm which profile is expiring. If an
