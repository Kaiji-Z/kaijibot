---
summary: "CLI reference for `kaijibot dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: kaijibot.internal)
title: "dns"
---
...
```bash
kaijibot dns setup
kaijibot dns setup --domain kaijibot.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `kaijibot.internal`)
...
- zone file path
- current tailnet IPs
- recommended `kaijibot.json` discovery config
...
- If `--domain` is omitted, KaijiBot uses `discovery.wideArea.domain` from config.
