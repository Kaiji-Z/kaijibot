---
summary: "CLI reference for `kaijibot onboard` (interactive onboarding)"
read_when:
...
```bash
kaijibot onboard
kaijibot onboard --flow quickstart
kaijibot onboard --flow manual
kaijibot onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`KAIJIBOT_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `kaijibot gateway run`.
...
- Fastest first chat: `kaijibot dashboard` (Control UI, no channel setup).
