---
summary: "CLI reference for `kaijibot qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
kaijibot qr
kaijibot qr --setup-code-only
kaijibot qr --json
kaijibot qr --remote
kaijibot qr --url wss://gateway.example/ws
```
...
- With `--remote`, KaijiBot requires either `gateway.remote.url` or
