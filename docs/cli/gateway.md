---
summary: "KaijiBot Gateway CLI (`kaijibot gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is KaijiBot’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `kaijibot gateway …`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.kaijibot/kaijibot.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
- `kaijibot onboard --mode local` and `kaijibot setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
- `--token <token>`: token override (also sets `KAIJIBOT_GATEWAY_TOKEN` for the process).
...
```bash
kaijibot gateway health --url ws://127.0.0.1:18789
```
...
```bash
kaijibot gateway usage-cost
kaijibot gateway usage-cost --days 7
kaijibot gateway usage-cost --json
```
...
kaijibot gateway status --require-rpc
```
...
```bash
kaijibot gateway call status
kaijibot gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
- For password auth on `gateway run`, prefer `KAIJIBOT_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
- In inferred auth mode, shell-only `KAIJIBOT_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
- If both `gateway.auth.token` and `gateway.auth.password` are configured and `gateway.auth.mode` is unset, install is blocked until mode is set explicitly.
- Lifecycle commands accept `--json` for scripting.

## Discover gateways (Bonjour)

`gateway discover` scans for Gateway beacons (`_kaijibot-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `kaijibot.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
...
```bash
kaijibot gateway discover --timeout 4000
kaijibot gateway discover --json | jq '.beacons[].wsUrl'
```
