...
service via the external `kaijibot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.kaijibot.gateway`
  (or `ai.kaijibot.<profile>` when using `--profile`/`KAIJIBOT_PROFILE`; legacy `com.kaijibot.*` is supported).
