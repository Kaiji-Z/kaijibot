...
- Same path: run the app from a fixed location (for KaijiBot, `dist/KaijiBot.app`).
...
```bash
sudo tccutil reset Accessibility ai.kaijibot.mac
sudo tccutil reset ScreenCapture ai.kaijibot.mac
...
Workaround: move files into the KaijiBot workspace (`~/.kaijibot/workspace`) if you want to avoid per-folder grants.
