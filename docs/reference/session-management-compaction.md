...
This document explains how KaijiBot manages sessions end-to-end:
...
KaijiBot is designed around a single **Gateway process** that owns session state.
...
KaijiBot persists sessions in two layers:
...
- Transcripts: `~/.kaijibot/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

KaijiBot resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, KaijiBot reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
kaijibot sessions cleanup --dry-run
kaijibot sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.kaijibot/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
KaijiBot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When KaijiBot splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, KaijiBot
...
- If a trailing tool-result block would otherwise push the chunk over target,
  KaijiBot preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (KaijiBot consumes the events, but Pi decides when to compact).
...
KaijiBot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, KaijiBot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, KaijiBot leaves it alone.
...
- If the provider fails or returns an empty result, KaijiBot falls back to built-in LLM summarization automatically.
...
- `kaijibot status` (CLI)
- `kaijibot sessions` / `sessions --json`
...
KaijiBot supports “silent” turns for background tasks where the user should not see intermediate output.
...
- KaijiBot strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, KaijiBot also suppresses **draft/typing streaming** when a
...
KaijiBot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but KaijiBot’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `kaijibot status`.
