...
This is the full reference for `kaijibot onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.kaijibot/kaijibot.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `kaijibot doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though KaijiBot now prefers Claude CLI reuse when available.
    - **OpenAI Code (Codex) subscription (Codex CLI)**: if `~/.codex/auth.json` exists, onboarding can reuse it. Reused Codex CLI credentials stay managed by Codex CLI; on expiry KaijiBot re-reads that source first and, when the provider can refresh it, writes the refreshed credential back to Codex storage instead of taking ownership itself.
    ...
    - Auth profiles live in `~/.kaijibot/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.kaijibot/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.kaijibot/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$KAIJIBOT_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.kaijibot/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `kaijibot pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `kaijibot configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `kaijibot health`.
    - Tip: `kaijibot status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
  </Step>
...
```bash
export KAIJIBOT_GATEWAY_TOKEN="your-token"
kaijibot onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env KAIJIBOT_GATEWAY_TOKEN
```
...
- Stores it under `~/.kaijibot/tools/signal-cli/<version>/`.
...
Typical fields in `~/.kaijibot/kaijibot.json`:
...
`kaijibot agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.kaijibot/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.kaijibot/agents/<agentId>/sessions/`.
