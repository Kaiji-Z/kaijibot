...
KaijiBot integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- KaijiBot owns lifecycle when `channels.signal.autoStart=true`.
...
- KaijiBot spawns `imsg rpc` as a child process (legacy iMessage integration).
