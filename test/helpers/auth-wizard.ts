...
export function requireKaijiBotAgentDir(): string {
  const agentDir = process.env.KAIJIBOT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("KAIJIBOT_AGENT_DIR not set");
  }
  return agentDir;
}
