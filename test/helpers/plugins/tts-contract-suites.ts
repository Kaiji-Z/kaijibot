...
function asLegacyTtsConfig(value: unknown): KaijiBotConfig {
  return value as KaijiBotConfig;
}

function asLegacyKaijiBotConfig(value: Record<string, unknown>): KaijiBotConfig {
  return value as unknown as KaijiBotConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): KaijiBotConfig {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    const baseCfg: KaijiBotConfig = asLegacyKaijiBotConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.KAIJIBOT_TTS_PREFS;
      process.env.KAIJIBOT_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.KAIJIBOT_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: KaijiBotConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
