...
export function isLocalCheckEnabled(env) {
  const raw = env.KAIJIBOT_LOCAL_CHECK?.trim().toLowerCase();
...
export function applyLocalTsgoPolicy(args, env, hostResources) {
  ...
  if (nextEnv.KAIJIBOT_TSGO_PPROF_DIR && !hasFlag(nextArgs, "--pprofDir")) {
    insertBeforeSeparator(nextArgs, "--pprofDir", nextEnv.KAIJIBOT_TSGO_PPROF_DIR);
  }
...
export function acquireLocalHeavyCheckLockSync(params) {
  ...
  const locksDir = path.join(commonDir, "kaijibot-local-checks");
  const lockDir = path.join(locksDir, `${params.lockName ?? "heavy-check"}.lock`);
  const ownerPath = path.join(lockDir, "owner.json");
  const timeoutMs = readPositiveInt(
    env.KAIJIBOT_HEAVY_CHECK_LOCK_TIMEOUT_MS,
    DEFAULT_LOCK_TIMEOUT_MS,
  );
  const pollMs = readPositiveInt(env.KAIJIBOT_HEAVY_CHECK_LOCK_POLL_MS, DEFAULT_LOCK_POLL_MS);
  const progressMs = readPositiveInt(
    env.KAIJIBOT_HEAVY_CHECK_LOCK_PROGRESS_MS,
    DEFAULT_LOCK_PROGRESS_MS,
  );
  const staleLockMs = readPositiveInt(
    env.KAIJIBOT_HEAVY_CHECK_STALE_LOCK_MS,
...
function readLocalCheckMode(env) {
  const raw = env.KAIJIBOT_LOCAL_CHECK_MODE?.trim().toLowerCase();
