...
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://kaijibot.ai/install.sh | bash
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://kaijibot.ai/install.ps1 | iex"
        exit 1
    fi
...
show_footer_links() {
    local faq_url="https://docs.kaijibot.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://kaijibot.ai/install.sh | bash -s -- [options]
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://kaijibot.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://kaijibot.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://kaijibot.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://kaijibot.ai/install.sh | bash"
}
...
main() {
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://kaijibot.ai/install.sh | bash -s -- --install-method npm"
