...
If still empty, check `/data/.clawdbot/kaijibot.json`:

```
cat /data/.clawdbot/kaijibot.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
CONFIG_PATH="${KAIJIBOT_CONFIG_PATH:-${KAIJIBOT_STATE_DIR:-$HOME/.kaijibot}/kaijibot.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/kaijibot.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/kaijibot.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")
