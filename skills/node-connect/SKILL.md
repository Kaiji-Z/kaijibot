---
name: node-connect
description: Diagnose KaijiBot node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify KaijiBot is advertising that route, then fix pairing/auth.
...
- whether `kaijibot devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `kaijibot qr --json`. It uses the same setup-code payload Android scans.

```bash
kaijibot config get gateway.mode
kaijibot config get gateway.bind
kaijibot config get gateway.tailscale.mode
kaijibot config get gateway.remote.url
kaijibot config get gateway.auth.mode
kaijibot config get gateway.auth.allowTailscale
kaijibot config get plugins.entries.device-pair.config.publicUrl
kaijibot qr --json
kaijibot devices list
kaijibot nodes status
```

If this KaijiBot instance is pointed at a remote gateway, also run:

```bash
kaijibot qr --remote --json
```
...
`kaijibot qr --json` success means:
...
If `kaijibot qr --json` says `Gateway is only bound to loopback`:
...
```bash
kaijibot devices list
kaijibot devices approve --latest
```
...
- `kaijibot devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run kaijibot qr again, rescan, then approve the pending device pairing.`
